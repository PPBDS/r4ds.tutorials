...
<!-- DK: Always spaces around the pipe |> -->
...
If you do not have access to the internet while doing this tutorial, you can replace "https://rvest.tidyverse.org/articles/starwars.html" with "data/star_wars.html" in all the questions below and everything should work.
...
```{r web-scaping-examples-1-test, include = FALSE}
"data/star_wars.html" |> 
  read_html()
```
...
```{r web-scaping-examples-2-test, include = FALSE}
"data/star_wars.html" |> 
  read_html()|> 
  html_elements("section")
```
...
```{r web-scaping-examples-3-test, include = FALSE}
"data/star_wars.html" |> 
  read_html() |> 
  html_elements("section") |> 
  html_element("h2")
...
```{r web-scaping-examples-4-test, include = FALSE}
"data/star_wars.html" |> 
  read_html()|> 
  html_elements("section")|> 
  html_element("h2") |> 
...
```{r web-scaping-examples-5-test, include = FALSE}
tibble(title = 
         "data/star_wars.html" |> 
         read_html() |> 
         html_elements("section") |> 
...
<!-- DK: Turn the below into one or two proper questions. First question, pipe http to read. Second, continue pipe to html_elements(). Third, we report that, behind the scenes, we jave assigned the result of this pipe to an object named "section." Then, we ask the question, "Type `section` and hit Run Code. -->

### Exercise 6

We can use the same approach to add other variables to this tibble, like the director and release date. To avoid code duplication, create an object called `section`, and set its value to `https://rvest.tidyverse.org/articles/starwars.html`
...
Behind the scenes, we have just assigned the result of this pipe to an object called `section`. Type in `section` and hit "Run Code."
...
DK: Copy the code from above which created a tibble with one column, `title`. Change the code so that blah. blah, bpah, is replaced with just `section`.

Now that we have an object, we can use it for our tibble. Create a tibble using `title` and `director` for its arguments. set title equal to `section |>  html_elements("h2") |>   html_text2()`, and director to `section |> html_element(".director") |>  html_text2()`.
...
```{r web-scaping-examples-11-test, include = FALSE}
"data/imdb.html" |> 
  read_html()
```
...
```{r web-scaping-examples-12-test, include = FALSE}
"data/imdb.html" |>
  read_html()|> 
  html_element(".chart")
```
...
```{r web-scaping-examples-13-test, include = FALSE}
"data/imdb.html" |> 
  read_html() |> 
  html_element(".chart") |> 
  html_table()
...
```{r web-scaping-examples-14-test, include = FALSE}
"data/imdb.html" |> 
  read_html()|> 
  html_element(".chart")|> 
  html_table()|> 
  select(rank_title_year = `Rank & Title`,
      rating = `IMDb Rating`)
```
...
```{r web-scaping-examples-15-test, include = FALSE}
"data/imdb.html"|> 
  read_html()|> 
  html_element(".chart")|> 
  html_table()|> 
...
```{r web-scaping-examples-16-test, include = FALSE}
"data/imdb.html" |> 
  read_html()|> 
  html_element(".chart")|> 
  html_table()|> 
...
```{r web-scaping-examples-17-test, include = FALSE}
"data/imdb.html" |> 
  read_html()|> 
  html_element(".chart")|> 
  html_table()|> 
...
```{r web-scaping-examples-18-hint-1, eval = FALSE}
"data/imdb.html"  |> 
  read_html() |> 
  html_element(".chart") |> 
  html_table() |>   
...
```{r web-scaping-examples-18-test, include = FALSE}
"data/imdb.html"  |> 
  read_html() |> 
  html_element(".chart") |> 
  html_table() |>   
  ...
  mutate(rank_title_year = str_squish(rank_title_year)) |> 
  mutate(rank = parse_number(rank_title_year)) |>   
  mutate(title = str_extract(rank_title_year,  pattern = "(?<=\\d\\.\\s)(.*)(?=\\s\\(\\d{4}\\))")) |> 
  select(rank, title, rating)
```
