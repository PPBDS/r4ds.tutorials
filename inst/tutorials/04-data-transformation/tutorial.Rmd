...
`filter()` changes which rows are present without changing their order. Pipe `flights` to `fliter(dep_delay > 120)` 
...
<!-- AB: No instructions given, maybe just add the last sentence of previous knowledge drop here -->
...
Create a new object, `jan1` which is the result of filtering `flights` to just including flights from January 1. In other words, put `jan1 <- ` in front of your answer to the Exercise above.
...
Continue the pipe to `mutate()`, creating a new variable `speed` which equals `distance / airtime`.

<!-- AB:Change airtime to `air_time` -->
...
When examining a new tibble, we need to do more than just look at the top and bottom. We should look at some randomly selected rows as well. Pipe `flights` to `slice_random()`, setting `n` to equal `6`.
<!-- AB: slice_random not workign for some reason -->
