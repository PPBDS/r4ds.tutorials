...
The `diamonds` dataset contains the prices and other attributes of almost 54000 diamonds, such as the `price`, `carat`, `cut`, `color`, and so on.
...
Let's visualize the distribution of the variable's values using a set binwidth.
...
Copy your previous code and create a pipeline with `diamonds`. Pipe `diamonds` with `filter()` and filter out `carat` that are less than `3`. 
...
ometimes outliers are data entry errors, sometimes they are simply values at the extremes that happened to be observed in this data collection, and other times they suggest important new discoveries.
