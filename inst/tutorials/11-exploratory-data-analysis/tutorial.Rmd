...
Within `filter()`, add `|` and also filter `y` to be greater than `20`.
...
<!-- SG: Is this the correct wording? -->
...
Run `install.packages("hexbin")` in the Console and Copy/Paste the results here.

```{r covariation-with-2-n-8}
question_text(NULL,
...
```{r covariation-with-2-n-13, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r covariation-with-2-n-13-hint-1, eval = FALSE}
... + 
  labs(
    title = ...,
    subtitle = ...,
...
```{r patterns-and-models-1-hint, eval = FALSE}
library(...)
```
...
```{r patterns-and-models-2-hint, eval = FALSE}
diamonds3 <- diamonds
```
...
```{r patterns-and-models-3-hint, eval = FALSE}
diamonds3 <- diamonds |> 
  mutate(
    log_price = ...
  )
...
```{r patterns-and-models-4-hint, eval = FALSE}
diamonds3 <- diamonds |> 
  mutate(
    log_price = log(price),
    log_carat = ...
...
```{r patterns-and-models-5-hint, eval = FALSE}
diamonds_fit <- ...
```
...
```{r patterns-and-models-6-hint, eval = FALSE}
diamonds_fit <- linear_reg() |> 
  ...
```
...
```{r patterns-and-models-7-hint, eval = FALSE}
diamonds_aug <- ...
```
...
```{r patterns-and-models-8-hint, eval = FALSE}
...
```{r patterns-and-models-9-hint, eval = FALSE}
diamonds_aug |> 
  ggplot()
```
...
```{r patterns-and-models-10-hint, eval = FALSE}
diamonds_aug |> 
  ggplot(aes(x = ...))
```
...
```{r patterns-and-models-11-hint, eval = FALSE}
diamonds_aug |> 
  ggplot(aes(x = carat, y = ...))
```
...
```{r patterns-and-models-12-hint, eval = FALSE}
... +
  geom_point()
```
...
```{r patterns-and-models-13-hint, eval = FALSE}
...
```{r patterns-and-models-14-hint, eval = FALSE}
... +
  labs(
    title = ...,
    subtitle = ...,
