...
p4 <- ggplot(mpg, aes(x = drv, y = cty, color = drv)) + 
  geom_boxplot(show.legend = FALSE) + 
  labs(title = "Plot 1")

p5 <- ggplot(mpg, aes(x = drv, y = hwy, color = drv)) + 
  geom_boxplot(show.legend = FALSE) + 
  labs(title = "Plot 2")

p6 <- ggplot(mpg, aes(x = cty, color = drv, fill = drv)) + 
  geom_density(alpha = 0.5) + 
  labs(title = "Plot 3")

p7 <- ggplot(mpg, aes(x = hwy, color = drv, fill = drv)) + 
  geom_density(alpha = 0.5) + 
  labs(title = "Plot 4")

p8 <- ggplot(mpg, aes(x = cty, y = hwy, color = drv)) + 
...
<!-- DK: Maybe move babynames example elsewhere? This tutorial is pretty long already! Took the last student 2 hours. -->
...
```{r labels-19-test, include = FALSE}
?plotmath #In the Console
```
...
<!-- DK: Change around. Show the plots that we have created. Then the questions can be about combining them. -->
