...
Pipe `penguins` into `ggplot()`, and within `ggplot()`, set `mapping = aes()`. Set `x` to `body_mass_g` and `y` to `flipper_length_mm`. Type `-n tail 7 Quarto-1.qmd`in terminal and CP/CR the answer here.
...
`-n tail 7 Quarto-1.qmd` gives you the last 7 lines of your `Quarto-1.qmd` document. You can change the amount of lines you see by changing the 7 to another number. 

### Exercise 10

Within`aes()`, set `color` to `species` to sort the graph by the species and add `geom_point` function with `+`. Type `-n tail 7 Quarto-1.qmd`in terminal and CP/CR the answer here.
...
Add `labs` with `+`and set `title` to `" Penguins Body Mass compared to Flipper Length"`, `x` to `"Body Mass"`, and `y` to `"Flipper Length"`.  Type `-n tail 10 Quarto-1.qmd`in terminal and CP/CR the answer here.
...
Now that we completed this, we need to publish this Quarto Document to Rpubs.
...
Create a new Quarto doc and name it `Quarto-2.qmd`. 
