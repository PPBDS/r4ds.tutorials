...
Within`aes()`, set `color` to `species` to sort the graph by the species.
...
Add `labs` with `+`and set `title` to `" Penguins Body Mass compared to Flipper Length"`, `x` to `"Body Mass"`, and `y` to `"Flipper Length"`.  
...
One more exercise completing the graph
...
Now, we need to publish this Quarto Document to Rpubs.
