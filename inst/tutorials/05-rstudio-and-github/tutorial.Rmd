...
Add a code chunk and within it put `library(tidyverse)`, `library(tidyverse)`, `library(learnr)`, `library(tutorial.helpers)`, `library(tidyverse)`, `library(palmerpenguins)`. Run code. Save the doc and name it Quarto-1.( Keep the .qmd). CP/CR everything in the quarto doc here.
...
Now, we'll make a plot with the `penguins` data set.

### Exercise 7

Pipe `penguins` into `ggplot()`, and within `ggplot()`, set `mapping = aes()`. Set `x` to `body_mass_g` and `y` to `flipper_length_mm`. 
