...
For this and later Exercises which will be completed in the Console, you may find it useful to open a temporary R script (`File -> New File -> R Script`). You can write your answer there and then run it in the Console with `Command/Ctrl + Enter`. This makes it easy to edit your code, often by adding more lines to the pipe, and then submit the answer to the next question.

<!-- DK: Explain this better? Show video? -->
<!-- TJ: Tried to add a screen recorded video but it would not work. -->
...
To determine total checkouts by year, add `.by = CheckoutYear` to the call to `summarize()` in the Console. After `summarize()` in the pipe, add `arrange(CheckoutYear)`. CP/CR.
