...
penguins |>
  ggplot(mapping = aes(x = flipper_length_mm, y = ..., ... = species))+
  geom_point()
```
...
penguins |>
  ggplot(mapping = aes(x = flipper_length_mm, y = body_mass_g, color = ...)) +
  geom_point() +
  geom_smooth(... = "lm")
```
...
penguins |>
  ggplot(mapping = ...(x = flipper_length_mm, y = body_mass_g)) +
...
Copy the previous code, add the `geom_smooth()` layer and within it, set `method` to `lm`. 
...
Below is what the final rplot will look like:
...
Before doing so, lets load the **palmerpenguins** package using `library()`.
