...
A `variable`: a quantity, quality, or property that you can measure, `value`: state of a variable when you measure it, `observation`: a set of measurements made under similar conditions.
...
```{r saving-plots-1-hint, eval = FALSE}
penguins |>
  ggplot()
```
...
```{r saving-plots-2-hint, eval = FALSE}
... |>
  ggplot(aes(x=..., y=...))
```
...
```{r saving-plots-3-hint, eval = FALSE}
... |>
  +
  geom_point()
```
...
```{r saving-plots-4-hint, eval = FALSE}
plot_geom <- ... |>
                  ggplot()+
                  geom_point()
```
...
```{r saving-plots-5-hint, eval = FALSE}
