...
Let's observe the `filter()` equivalent. On a new line, pipe `df0` (with the `|>`) to the filter function. Pass in `x > 1` into `filter()`.
...
`arrange()` is equivalent to subsetting the rows with an integer vector, usually created with order(). In the code chunk below, pipe `df0` to `arrange()`, passing in `x, y`. After, on a new line, paste its equivalent: `df0[order(df0$x, df0$y),]`
