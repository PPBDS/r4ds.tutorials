...
<!-- DK: Fix exercise code blocks with exercise = TRUE. Use the template in the future. -->
...
Extract the first element of `x`, using the `x[i]` syntax.

```{r subsetting-with-a-si-2, exercise = TRUE}

```

```{r subsetting-with-a-si-2-hint-1, eval = FALSE}
x[...]
```
...
Along with subsetting logical, character, and integer vectors, you can also subset nothing. For example, by running `x[]` on the code above, it'll just return the vector `x`.


### Exercise 7

Subsetting can not only be used on vectors; they work on datasets too. There are many ways to use `[` with datasets, but the most common way is to subset by selecting rows and columns, with the syntax `df[row, col]`. Using the tibble below, try and extract the letter "a". **Hint: To make it easier to visualize, print the tibble on a separate line before extracting the letter**
