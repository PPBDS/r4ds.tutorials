...
```{r subsetting-with-double-brackets-10, exercise = TRUE}
tb2 <- tibble(x1 = 1)
tb2$x
```
...
```{r subsetting-with-double-brackets-10-test, include = FALSE}
tb2 <- tibble(x1 = 1)
tb2$x
```
...
<!-- LD: Add exercises and follow steps from the book.  -->
...
Use `list()` to create a list of values from 15 to 19. Save that into variable `L`. Then use a `for` loop to print out each element of list `L`.
...
Create a `list()` with numbers from 6 to 10 and save it in `L1`.
...
As you can see, the `print()` displayed the values inside `L` in the console.

<!-- LD: This part is a bit confusing -->
...
<!-- LD: Could provide more clarification on walk() -->
