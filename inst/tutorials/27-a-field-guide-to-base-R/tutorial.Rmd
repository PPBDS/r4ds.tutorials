...
# TJ: Was not able to fix this error
...
We see 5 different levels of `cut`: Fair, Good, Very Good, Premium and Ideal. Now lets recreate the code using `pull()`
...
Another important member of the apply family is `tapply()` which computes a single grouped summary. Lets use this on the **diamonds** package. In the code chunk below, pipe `diamonds` to `group_by(cut)`. Then continue the pipe to `summarize()` with the argument `price = mean(price)`
...
<!-- TJ: There are no more exercies in the book. -->
...
<!-- TJ: This tutorial seems alright, is there something I am missing? -->
