...
Use `write_csv()` to write the contents of the `students` object to a file called `"students.csv"'. Do this by setting the first argument of `write_csv()` `x` to `students` and then the second argument `file` to  `"students.csv"'
...
```{r writing-to-a-file-1-hint-1, eval = FALSE}
write_csv(x = ..., 
          ... = "students.csv")
```
...
As with many commonly used functions, we will often drop the argument names. In this case, we would typically write `write_csv(students, "students.csv")`.
...
```{r writing-to-a-file-3-hint-2, eval = FALSE}
write_rds(iris_p, 
          "test_1.rds")
```
...
```{r writing-to-a-file-6-hint-2, eval = FALSE}
