...
If you have already loaded the **tidyverse** package in this session, and have not restarted R, then you will see no output because RStudio has already provided you with this message.
...
Comments begin with hashmarks: `#`. R will ignore everything after the hashmark when trying to run the code.
...
`ggplot()` will generate only a blank plot until a `geom` layer is added. Note that you might get a warning when using `readLines()` if `script-2.R` does not end with a blank line. This warning does not matter to us but it is always good practice to end a text file with a blank line.
...
This should return `quarto-1.qmd`, as well as `analysis-1.Rproj`, `script-1.R`, `script-2.R`, and `hist-harvard.png`. Note how different suffixes are used for different types of files.
