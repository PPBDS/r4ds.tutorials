...
description: "Introduction to R scripts and Quarto. Covers material in Chapter 3
  Workflow: basics, in Chapter 5 Workflow: code style, in Chapter 7 Workflow: scripts and projects, and in Chapter 29 Quarto."
---
...
<!-- Remove references to all.primer.tutorials. Only refer to R4DS. -->

<!-- Stop using primer.data. -->
...
<!-- Standardize how we refer to short cut keys. Is "Cmd/Cntrl + s" good? Probably not. We should use the same terminology as the RStudio docs. Note that there are new shortcut codes in Quarto to make these display in a cool way. -->
...
On the next line, start a `ggplot()`, setting `data` to `diamonds` and mapping `x` to `caray` with the following code.
