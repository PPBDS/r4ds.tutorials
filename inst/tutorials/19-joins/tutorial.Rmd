...
Run `flights$origin - flights$time_hour`.
...
```{r basic-joins-1-test, include = FALSE}
flights2 <- flights |> 
select(year, time_hour, origin, dest, tailnum,carrier)
```
...
```{r basic-joins-2-test, include = FALSE}
flights2 |>
  left_join(airlines)
```
...
```{r basic-joins-3-test, include = FALSE}
flights2 |> 
...
```{r basic-joins-4-test, include = FALSE}
flights2 |> 
...
```{r basic-joins-5-test, include = FALSE}
flights2 |> 
...
To demonstrate the flaw, join the `flights2` with the `planes` dataset using `left_join()`.
To do so, start of by creating pipe with `flights2` to `left_join()` and pass in `planes` as the argument.
...
```{r basic-joins-6-test, include = FALSE}
flights2 |>
  left_join(planes)
```
...
```{r basic-joins-7-test, include = FALSE}
flights2 |>
...
```{r basic-joins-8-test, include = FALSE}
flights2 |>
...
```{r basic-joins-9-test, include = FALSE}
flights2 |>
...
```{r basic-joins-10-test, include = FALSE}
airports |> 
...
```{r basic-joins-11-test, include = FALSE}
flights2 |> 
...
```{r basic-joins-12-test, include = FALSE}
flights2 |> 
...
To describe a join type, we use dots to indicate matches. The matches determine the output rows, which include the key, x values, and y values. The image below displays an inner join, retaining rows only when the keys are equal.
...
```{r how-do-joins-work-9-test, include = FALSE}
df1 <- tibble(key = c(1, 2, 2), val_x = c("x1", "x2", "x3"))
df2 <- tibble(key = c(1, 2, 2), val_x = c("x1", "x2", "x3"))
```
...
```{r how-do-joins-work-10-test, include = FALSE}
df1 |> 
...
```{r nonequi-joins-4-test, include = FALSE}
...
```{r nonequi-joins-5-test, include = FALSE}
df <- tibble(name = c("John", "Simon", "Tracy", "Max"))
df |> 
  cross_join(df)
```
...
```{r nonequi-joins-7-test, include = FALSE}
...
Rolling joins return the closest match instead of all matches satisfying the inequality. To create a rolling join add `closest()` to the inequality condition. For example, `join_by(closest(x <= y))` finds the smallest `y ≥ x`, and `join_by(closest(x > y))` finds the largest `y < x`. Example:
...
```{r nonequi-joins-9-test, include = FALSE}
...
Let's now create a new tibble `employees` using `tibble()`. The tibble contains two columns: `name` and `birthday`. Pass in `name = sample(babynames::babynames$name, 100)`, which assigns a sample of 100 baby names to `name`. Then pass `ymd("2022-01-01") + (sample(365, 100, replace = TRUE) - 1)`, which generates birthdays, to `birthday`. 
...
```{r nonequi-joins-10-test, include = FALSE}
employees <- tibble(
  name = sample(babynames::babynames$name, 100),
...
We generated the `employees` tibble using the `babynames` dataset and also generated their birthdays. Let's now use rolling joins to find the first date that comes after their birthday.
...
```{r nonequi-joins-11-test, include = FALSE}
employees <- tibble(
     name = sample(babynames::babynames$name, 100),
     birthday = ymd("2022-01-01") + (sample(365, 100, replace = TRUE) - 1)
)
...
This approach only looks for birthdays after January 10th and leaves out birthdays before that date. To demonstrate this, copy the previous code and change the join from `left_join()` to `right_join()`.
...
```{r nonequi-joins-12-test, include = FALSE}
employees |> 
...
To resolve that issue we’ll need to tackle the problem a different way, with overlap joins: the fourth type of non equi join.
...
```{r nonequi-joins-13-test, include = FALSE}
parties2 <- tibble(
  q = 1:4,
  party = ymd(c("2022-01-10", "2022-04-04", "2022-07-11", "2022-10-03")),
  start = ymd(c("2022-01-01", "2022-04-04", "2022-07-11", "2022-10-03")),
...
```{r nonequi-joins-14-test, include = FALSE}
parties2 |> 
...
```{r nonequi-joins-15-test, include = FALSE}
parties2 |> 
  inner_join(parties2, join_by(overlaps(start, end, start, end), q < q)) |>
...
```{r nonequi-joins-16-test, include = FALSE}
parties4 <- tibble(
  q = 1:4,
  party = ymd(c("2022-01-10", "2022-04-04", "2022-07-11", "2022-10-03")),
  start = ymd(c("2022-01-01", "2022-04-04", "2022-07-11", "2022-10-03")),
...
```{r nonequi-joins-17-test, include = FALSE}
employees |> 
      inner_join(parties3, 
