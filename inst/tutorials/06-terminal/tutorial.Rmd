...
<!-- First, explain that directory separator is \ in windows and / elsewhere. Windows users must adjust the instructions accordingly. -->

<!-- Add an exercise which downloads a file from the web. This is good practice for the class session doing the same. -->

<!-- Lots of opportunities for knowledge drops. See Chapter 7 in R4DS. -->

<!-- Chapter 7 includes some discussion about relative and absolute paths. Include some of that stuff here? Certainly we should emphasize that you should only ever use relative paths, not absolute paths. How about the **here** package? -->
...
```{r}
knitr::include_graphics("images/pwd.png")
```
...
Let's **m**a**k**e a **dir**ectory called `my-class`. Run `mkdir my-class`. Run `ls` to confirm that the new directory exists inside the working directory. CP/CR.
...
`mv` stands for **m**o**v**e. The first argument is the file we want to move, the second argument is the location (both the path, if applicable, and filename) we want to move it to. Here the places have the same folder location --- the current working directory, or `.` --- but different names. We will move things to different directories with `mv` later.

### Exercise 7

Let's **r**e**m**ove the file `fake.txt`. Run `rm fake.txt`. CP/CR.
...
**Be careful** with the `rm` command in the Terminal. Unlike moving files to Trash on your laptop, it is (usually) **irreversible**.

### Exercise 8

Let's remove the directory `my-class`. Run `rm -r my-class`. CP/CR.
...
You will practice using paths both inside and outside the working directory. You will learn a few shortcuts to make this easier.
...
The Output pane (bottom right corner) in RStudio shows the contents of the default working directory, generally the project directory. Towards the top, you will see a series of directories, separated by `>`, on your computer, beginning with the Home icon. This should match the path generated by `pwd`.
...
The same command will have very different results depending on where you "are" in your computer. The location from which new commands are executed is the (present) working directory.
...
Because we have changed working directories, the new directory is now created directly inside the `lessons` directory.
...
We could not, as before, simply give `fruits` as an argument to `mkdir` because then it would be created directly inside `paths`, the current working directory, rather than within `lessons`. To refer to a location other than the working directory, we need to use a **path**, which describes the location directory by directory. The path above is called a **relative path** because it still assumes the working directory as its starting point.

### Exercise 6

Let's change our working directory to `fruits`. Run `cd lessons/fruits`. As you are typing this command, type only the `f` in `fruits` and then press the tab key (twice may be necessary on some computers). Pressing tab autocompletes the name of a file or directory. CP/CR.
...
Use `touch` to make a text file inside `fruits`, named `pineapple.txt`. Recall that your current location --- i.e., your working directory --- is still `lessons`. So, to make a new file within `fruits`, you need `touch fruits/pineapple.txt`. CP/CR.
...
Let's move the file `does-not-belong` so that it is directly inside of the working directory. Run `mv paths/lessons/fruits/does-not-belong .`. Recall that the first argument is the place from where we want to move the file. The second argument is the place where we want to move the file to. This might be just a filename, if we are giving the file a new name in the current directory. Or it might be a path and filename combination.
...
Other than the `.`, avoid beginning filenames with special characters or including spaces. These are more difficult to work with in Terminal.
...
We have already gone over a few shorthands that stand for paths. The **environmental variable** `R_HOME` represents the path to a folder of set-up files that R has installed on your computer. Type and enter `echo $R_HOME` alone to make sure we are telling you the truth. Then change your working directory to `$R_HOME` with `cd $R_HOME`.
...
Use `pwd` to confirm that you are still in `R_HOME`. Use `ls` to see what is inside `R_HOME`. You should see several unfamiliar items. Because this directory is unfamiliar, we may want more information. Let's run `ls -l`, so that we can see more information in a **l**ong list.
...
Like `mv`, `cp` can change a filename. Like `mv`, if the destination filename is omitted, `cp` will retain the same name by default.
...
Use `pwd` to confirm that you are in your default working directory. Make a directory named `wildcards`. CP/CR.
...
Using `touch`, Make three text files named `hat.txt`, `cat.txt`, and `ate.txt`. CP/CR.
...
Though `*` will match strings of any length, its positioning matters. `a*` is not the same thing as `*a`. The first matches any string (i.e., any filename) which starts with `a`. The second matches any string which ends with `a`.
...
Now let's try to make a list of all the files whose names before the suffix end in `t`. Run `ls *t.*` CP/CR.
...
First, make a new directory called `suess` within the `wildcards` directory in which you are currently located. 
...
The `wildcards` directory, in which you are currently located, now includes 5 items, one of which is the `suess` directory.
...
The `-l` option provides a **l**ong-format listing of the contents of the directory, including information about the type of content (file versus directory) and creation date/times.
...
Use the previous command (since we have confirmed that it generates the list of files we want to move) but replace `ls` with `mv` and then add `suess`, which indicates the location to which we want to move the files. In other words, run `mv *t.* suess`. CP/CR.
...
The `-R` option to the `ls` command stands for **r**ecursive. We see, not simply the contents of the current directory (two files and the `seuss` directory) but also the contents of the `suess` directory itself.
...
In the previous section, we used **wildcards** to create **regular expressions** in order to generate lists of filenames which matched a specific pattern. In this section, we will use `grep` to look for words inside of files. Fortunately, wildcards and regular expressions work the same with `grep`.
...
We call this folder `grepping` rather than `grep` you should not use the word for a command as the name of a file or directory.
...
Don't worry about the `printf` command. It is not used very often. We didn't use `echo` because it is annoying to use in the case of multiple lines of input where you are writing code which needs to work on both Mac/Unix and Windows.
...
This should return the line with `ate.txt`.
...
We will not attempt the previous search we made of these files with the wildcards because it is difficult, though possible, to search for strings with special characters like `.` with `grep`.
...
The `>` symbol is known as the "output redirection operator." Because all the output has been sent into `RHOME.txt`, this command produces no output.
...
The first number returned by `wc` is the number of lines in the file. The second is the total number of words. The third is the number of characters.
...
A mistyped `grep` command can cause the Terminal to get stuck and not generate a new prompt. If this happens, navigate to the top of the Terminal page and click the dropdown menu where Terminal says busy. Press Interrupt Terminal Command to generate a new prompt and start again.
...
`^` and `$` are called metacharacters. It is sometimes useful to put quotes around a regular expression which includes metacharacters.
...
An even more powerful version of `grep` is known as `egrep`. Learning more about `egrep` is beyond the scope of this tutorial.

### Exercise 12

Use `cd ..` to change the working directory to your default working directory. Run `pwd`. CP/CR.
