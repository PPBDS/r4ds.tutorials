...
Wildcards can be used with more than `ls`. Let's move `hat.txt` and `cat.txt` together into one directory. Make a new directory called `suess` within the `wildcards` directory in which you are currently located. Run`ls\`. CP/CR
...
The `echo` command simply "echoes" back what argument you pass it. In this case, we want to fill `files.txt` with the same list of filenames which we used before. Note that we have included an extra blank line at the end. It is always a good idea for text files to end with a blank line.
...
Run `ls $R_HOME` to make a list of the contents of the R home directory. CP/CR.
