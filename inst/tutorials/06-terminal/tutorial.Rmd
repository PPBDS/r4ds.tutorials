...
* Creating new files can work differently on Mac/Linux and Windows. To create a new empty file named `new.txt`, we would use `touch new.txt` on Mac/Linux. Assuming that you have installed [RTools](https://cran.r-project.org/bin/windows/Rtools/), `touch` should also work on Windows. If it doesn't work for you, try `copy NUL new.txt` to create an empty file named `new.txt`.
