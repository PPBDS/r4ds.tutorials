...
<!-- DK: Can you avoid using group_by -->

<!-- DK: `dbplyr` should be **dbplyr** -->
...
Create a pipe with the `mpg_db` use `group_by()` on `displ`.
...
Continue the pipe with `summarize()` and set `avg_hwy` to the `mean()` of `hwy` and `na.rm = TRUE`.
