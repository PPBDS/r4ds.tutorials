...
Copy previous code. Within `{}`, start a pipe with `df` and pipe it into `mutate(..., .keep = "none")`.
...
Finish the pipe in the brackets by adding `show_query()`.
...
Start a pipe with `flights_db` and pipe it into the `mutate_query()` variable we used from above.
