...
<!-- DK: Add an exercise which deletes the database once we have answered the last questins which uses it. OK if it is just this code and an instruction to press the button. -->
...
Let's use the `flights_db` variable from above. Create a pipe with `flights_db` to `select()` `carrier`, `tailnum`, `origin`, `distance`, and `year`.
...
Start a pipe with `flights_db`. Let's use `left_join()` and set `rename(year_built = year)` to `planes_db` in `left_join()`.
