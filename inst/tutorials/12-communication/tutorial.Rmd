...
Note: This plot is empty.

### Exercise 4

Within `ggplot()`, using `aes()`, set the `x` argument to the `displ` variable.
...
Within `ggplot()`, using `aes()`, set the `y` argument to the `hwy` variable.
...
The `se` argument removes the gray background of the line made through the function `geom_smooth()`. Our graphic is currently exploratory, let's turn it into an expository graphic with some good labels.
...
Type in `labels_tib` and hit "Run Code". 
...
Within this function, using `aes()`, set the `x` argument to the `start` variable.
...
Within this function, using `aes()`, set the `y` argument to the `end` variable.
...
Set the `x` axis title to `quote(x[i])` by adding `labs()` to the pipeline
...
In this section we will learn how to label an individual observation or a group of observations instead of just labeling major components of a plot. 
