...
Let's check if altitude and latitude are good primary keys for airports, so copy the previous code and change the datset to `airports` and change the arguement withint `count()` to `alt , lat`.
...
That said, we might be better off introducing a simple numeric surrogate key using the row number instead of confusing the airliens and customers by using three columns are primary keys. 

Let's make a new primary key `id` and to add a new column `id` with row numbers to the flights dataset, start a pipe with flights and use mutate() function. Assign row_number() function to the id column and set .before to 1.
...
Copy the previous code and within `left_join()`, add `join_by()` and pass in `tailnum` as the arguement.
...
As we saw `join_by()` gives us the flexibilty to join  based on the variables we want. For example we know that we can join `flights2` and `airports` by `dest` or `origin`.
...
Let's now use `anti_join()`, so copy the previous code and the 
...
Compared to `semi_join()` , Anti joins are the opposite: they return all rows in x that don’t have a match in y.

### Exercise 12

 Let's find rows that are missing from airports by looking for flights that don’t have a matching destination airport usign `anti_join()`. First start a pipe with `flights2` to `anti_join()` which takes in `airports` as the joining tibble and we will tell it to join by `dest == faa`.
...
Now create a pipe with `df1` to `inner_join()` and pass in `df2` and then join the tibble by `key` using `join_by()`.
...
Copy the previous code and on a new line start a pipe with `df` to `cross_join()` and pass in `df` as the arguement.
...
Now that we have the dates ready, let's marcg each employee to their party.

### Exercise 17

Let's join `employees` and `parties` by first create a pipe with `employees` to `inner_join()`. We will be joining them with `between(birthday, start, end)` using `join_by()` and in addition also add `unmatched = "error"`.
