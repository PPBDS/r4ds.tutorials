...
Let's join `employees` and `parties` by first create a pipe with `employees` to `inner_join()`. We will be joining them with `between(birthday, start, end)` using `join_by()` and in additoin also add `unmatched = "error"`.
