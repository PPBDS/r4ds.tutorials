...
```{r tidy-data-6-hint, eval = FALSE}
table2
```

###

`table2` is a tibble with 4 columns and 12 rows. Let's see if the rules apply.### Exercise 7
...
```{r lengthening-data-1-hint, eval = FALSE}
billboard
```
...
This data is going to be extremely difficult to work with. It has *79* rows!
...
```{r lengthening-data-2-hint, eval = FALSE}
...|>
```
...
```{r lengthening-data-3-hint, eval = FALSE}
billboard |>
  pivot_...()
```
...
```{r lengthening-data-4-hint, eval = FALSE}
billboard|>
...
```{r lengthening-data-5-hint, eval = FALSE}
billboard|>
  pivot_longer(cols = starts_with("wk"),
             names_to = "...",
             values_to = "...")
...
```{r lengthening-data-6-hint, eval = FALSE}
pivot_longer(...,
             ...,
             ...,
             values_drop_na = ...)
...
```{r lengthening-data-7-hint, eval = FALSE}
(...)|>
  mutate()
```
...
```{r lengthening-data-8-hint, eval = FALSE}
|>
  mutate(... = parse_number(...))
```
...
```{r lengthening-data-9-hint, eval = FALSE}
billboard_longer <- billboard |>
  pivot_longer(...)
```
