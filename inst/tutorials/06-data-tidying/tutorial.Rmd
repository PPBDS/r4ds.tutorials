...
Let's  take a look at `table2` first. Run the code in the box below to see it.
...
```{r tidy-data-7-2}
question_text("Are all the rows observations?",
	answer(NULL, correct = TRUE),
	allow_retry = TRUE,
	try_again_button = "Edit Answer",
	incorrect = NULL,
...
Your answers should be "No" for both questions. 
...
Now let's look at `table3` Run the code in the box below to see it.
...
Your answer should be "No". Each of the cells in the `rate` column are not a single observation. They are `population` divided by `cases`, so in order for the data to be considered tidy, we would need to separate those columns.

### Exercise 9

Let's look at `table1`. Run the code in the box below to see it.
...
Both of your answers should be "Yes" because this is a very good example of a tidy dataset. All the variables are columns. All the columns are variables. All the observations are rows, all the rows are observations, all the values are cells, and all the cells are single values.
...
```{r tidy-data-11-hint, eval = FALSE}
ggplot(data = ...)
```
...
```{r tidy-data-12-hint, eval = FALSE}
...
```{r tidy-data-13-hint, eval = FALSE}
ggplot(...(...)) + 
  geom_...()
```
...
```{r tidy-data-14-hint, eval = FALSE}
ggplot(...(...)) + 
  geom_line(aes(group = country))
```
...
```{r tidy-data-15-hint, eval = FALSE}
ggplot(...(...)) + 
  geom_...(...) +
...
```{r tidy-data-16-hint, eval = FALSE}
ggplot(...(...)) + 
  geom_...(...) +
  geom_...(...) +
...
```{r tidy-data-17-hint, eval = FALSE}
ggplot(...(...)) + 
  geom_...(...) +
  geom_...(...) +
  scale_...(...) +
...
```{r tidy-data-18-hint, eval = FALSE}
... +
  labs(title = "...",
       subtitle = "...",
       x = "...",
...
Now that you understand what tidy data is, we need to learn how to make data tidy.
