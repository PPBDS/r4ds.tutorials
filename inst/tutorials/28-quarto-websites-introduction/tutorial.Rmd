...
In the Source pane, open the `_quarto.yml` file. If the `title` field is not correct, often because it just contains a `.`, replace `title: "."` with `title: "A New Website"`. Save the file. 
...
Stop the `quarto preview` command by hitting the red stop sign on the upper right of the Terminal tab. From the Terminal, run `quarto publish`. You may need to hit `return/enter` to confirm that you still want to use GitHub Pages. 
