...
<!-- Change this so it all works for Windows users. Perhaps that involves mentioning Windows equivalents for all commands in the text. https://www.geeksforgeeks.org/linux-vs-windows-commands/. Perhaps we can use commands like Sys.getenv("R_HOME") to establish the directory location and then use that location when issuing commands in the Terminal. -->

<!-- Note that Relative and absolute paths are discussed in Chapter 7 of R4DS. -->
...
This tutorial will introduce the structure and uses of the Terminal. (Note that Windows is different from Mac or Linux. The default commands in this tutorial assume that are using Mac/Linux. If you are in Windows, translate the commands using [this resource](https://www.geeksforgeeks.org/linux-vs-windows-commands/).)
