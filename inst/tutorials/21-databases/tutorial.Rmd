...
Type `as_tibble()`. Within `as_tibble()`, add `dbGetQuery()` with the arguments `con` and `my_sql`
...
`tbl()` is a function in **dtplyr** which creates a table. Note that a table is not the same thing as a tibble. Note the message at the top:
