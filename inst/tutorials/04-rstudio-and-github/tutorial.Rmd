...
<!-- Project 1 completes, but plot is ugly. -->

<!-- Project 2 makes same plot nice, and learns about code chunk options and execute. -->

<!-- Project 3 -->

<!-- Project 4: Make any plot they like! -->

<!-- Each project does the start up process more quickly, with less detail. -->

<!-- Each project begins with repo/project. Then does base QMD/html. Then edit .gitignore and commit/push. If we had fourth project, these 6 steps might just be the first question! Always finish by publishing something. Always end with a publish, followed by a final commit/push. -->

<!-- First project: Make the plot slowly. Do the interactive stuff in the Console. Show how we build together. -->

<!-- Second project: Start with the code from the preview project. (Include it, at least the plot, in the question.) Then explore how changes in chunk options changes the output.  Get rid of uglyness and warnings. -->
...
```{r}
knitr::include_graphics("images/rstudio_panes.jpg")
```
...
<!-- DK: Failed to discuss example_files/ directory. -->
...
To produce a complete report containing all text, code, and results, click “Render” or press `Command/Ctrl + Shift + K`. You can also do this programmatically with `quarto::quarto_render("quarto-1.qmd")`, although that requires installing the **quarto** package first. 
...
The `quarto-1.html` is our rendered file, as expected. The `quarto-1_files` directory contains a variety of files which were involved in the transformation of `quarto-1.qmd` into `quarto-1.html`. None of those files are worth understanding, at least at this point in your education. 

Nor are those files worth saving on Github. They are neither the original source material (`quarto-1.qmd`) which we will edit to make changes nor the final product (`quarto-1.html`) which we will show to others.
...
Everytime that you render a Quarto document, RStudio creates a directory in which it places the supporting files which we used in that rendering. The name of that directory is `name-of-your-file-without-the-suffx` plus `_files`. The material in this directory is not worth backing up on GitHub. It is neither the source file which we need to edit to make changes nor the output file which we will show to people.
...
````
> readr::read_lines("quarto-1.qmd") |> cat(sep = "\n")
---                                                                                                                                    
title: "Quarto 1"
author: "David Kane"
format: html
...
Change the code to pipe `penguins` into `ggplot()`, and within `ggplot()`, set `mapping = aes()`. Set `x` to `body_mass_g` and `y` to `flipper_length_mm` within `aes()`. Save the file. Render the document.
...
Commit the changes in `quarto-1.qmd` and `quarto-1.html`. Ignore other files and directories like `rsconnect`. (We will discuss them in the next section.) Use a sensible commit message. At the Terminal, run `git log`. 
...
<!-- Change the name from "Document" to "My First Quarto Document". Deal with slug -->
...
Select `File -> New File -> Quarto Document ...`. Provide a title ("Quarto 2") and an author (you). Save the document with a sensible name. Render the document.
...
Edit the `.gitignore` by adding `*Rproj` and `*_files`. Commit and push the remaining files.
...
Second, it shows the startup message printed by the **tidyverse** package. This is 
...
We need to make it so that we do not see this code or the output in the render `quarto-1.html` document. To do this, we will put `#| echo: false` and `#| message: false` before `library(tidyverse)` in the first code chunk. Render.

In the Console, run `readr::read_lines("quarto-2.qmd") |> cat(sep = "\n")`. CP/CR.
...
The `#| echo: false` makes it so that the code in the code chunk --- in this case, the code which create our plot --- does not appear in the rendered HTML.
...
Delete `#| echo: false` from the two code chunks in 
...
Create a Quarto doc. Render it. Edit your `.gitignore` appropriately. Commit/push everything. From the Terminal, run `git log`. CP/CR.
...
Again, note how we are speeding things up, not explaining each little step. By this time, you should be able to go from nothing to a properly set up GitHub repo and R project. 

Did you remember to give your Quarto document a sensible title (perhaps "Quarto 3") and an author (presumably you)? Did you add `*Rproj` and `*_files` to your `.gitignore`? If not, fix those things now.

<!-- ## Project 4 -->
<!-- ###  -->

<!-- Could be: hear is code for a plot. Do all the steps! Answer is two things: URL for Rpubs and git log result. -->
