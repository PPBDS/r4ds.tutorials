...
**Note:** Make sure you received the expected outputs of the name and email associated with your Github account from the last two exercises.
...
Open the `.gitignore` and add a new line with `First_Repo.Rproj`. This tells git to ignore any file named `First_Repo.Rproj`. We could also have used `*.Rproj`. The \* tells your computer that we want to prevent all files ending in `.Rproj` from being uploaded.
...
You should see `example.qmd` somewhere in the output. Note that the title inside of the document ("Example") has no necessary connection to the name of the file (`example.qmd`).
...
We now understand the entire data science work cycle. Begin with Github repository, connect to an RStudio project, create a Quarto document (including a plot or another analysis), and, finally, publish your results to RPubs. Let's practice these schools in new projects.
...
Go to `File -> New File -> Quarto Document`. Name it `Quarto-1`. RStudio will automatically add the `qmd` suffix. From the Terminal, run `ls`. CP/CR.
...
To produce a complete report containing all text, code, and results, click “Render” or press `Cmd/Ctrl + Shift + K`. You can also do this programmatically with `quarto::quarto_render("Quarto-1.qmd")`. This will reate an HTML file and display it in the viewer pane.

### Exercise 4

Switch to "Source". Delete everything in `Quarto-1.qmd` except the top box with title, format, and editor. CP/CR everything in `Quarto-1.qmd` here.
...
<!-- DK: I don't understand this comment. -->
...
<!-- Commented out project 2 because I didn't have time to fix it up before the release. -->
