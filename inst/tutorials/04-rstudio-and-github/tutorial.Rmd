...
<!-- Key issue: How long is this tutorial? How long is enough? Do we have room to add a Project 3 (and a Project 4)? -->

<!-- Publishing: Should we be exploring other ways, besides RPubs, of publishing pages? quarto publish quarto-pub quarto-1.qmd works! quarto publish gh-pages quarto-1.qmd also has real possibilities.  -->

<!-- Project 3 Make any plot they like! Maybe using diamonds. Maybe give some advice? Maybe a fork? Maybe Github pages? Would require creating an index.qmd. -->

<!-- Project 4:  Totally at student's discretion? Maybe explain how to use the visual editor. Also, review all the shortcut keys we learned in RStudio and Code.-->
...
We have forgotten to label the code chunks. Not good! Add `#| label: setup` to the first code chunk and `#| label: make-plot` to the second. Save the file. Render it.

From the Termina, run `grep label quarto-2.qmd`. CP/CR.
...
````
Davids-MBP:project-2 dkane$ grep label quarto-2.qmd
# | label: setup
# | label: make-plot
Davids-MBP:project-2 dkane$ 
````

`quarto-2.html` is unchanged. Labelling code chunk does not affect the output, but it is still good practice.
...
From the Terminal, run `grep "#|" quarto-2.qmd`. CP/CR.
...
````
Davids-MBP:project-2 dkane$ grep "#|" quarto-2.qmd
# | message: false
# | label: setup
# | label: make-plot
# | warning: false
...
Add `rsconnect` to `.gitignore`. Commit and push `quarto-2.qmd` and `quarto-2.html`. Use a sensible commit message like "published version." 
...
<!-- Could make this a project which they fork! -->

<!-- Could teach caching? What other tricks are useful for students?  -->
