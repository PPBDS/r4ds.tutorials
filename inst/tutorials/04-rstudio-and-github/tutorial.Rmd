...
<!-- Restart R with shortcut key at the start of every Quarto X section. Actually, this happens naturally with the new project in each section. -->
...
First let's make a new Quarto document. Select `File -> New File -> Quarto Document ...`. Change the title to "Example" and keep the other fields as the default values. Save the file with `Command/Ctrl - s` and name the file `example`. RStudio will automatically add the `.qmd` suffix.

### 

Run `list.files()`. CP/CR.

```{r publishing-to-r-pubs-ex-1}
question_text(NULL,
    answer(NULL, correct = TRUE),
    allow_retry = TRUE,
    try_again_button = "Edit Answer",
...
Connect the `project-2` Github repo to an R project on your computer. Name the R project `project-2` also. (Don't forget the Tab-key trick.) Keeping the names of repos/projects aligned makes organization simpler. From the Terminal, run `ls`. CP/CR.
...
Go to File, New File, Quarto Document. Name it `Quarto-2`. From the Terminal, run `ls -a`. CP/CR.
...
<!-- Knowledge Drop -Explain -a. Discuss dot files and directories. -->
