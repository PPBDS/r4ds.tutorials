...
The next step will be to create the exact same project in your local computer through the process of **cloning**. By having a local and a GitHub version, you can edit your project on your computer and send all the changes you've made to GitHub. This process insures that the GitHub version is synced up with your local version.
...
Restart your R session with `Session -> Restart R`. (This is not the same thing as restarting RStudio.) You could also use `Command/Ctrl + Shift + 0` to restart R. Using the short-cut key is always a good idea.
...
If the error "could not find slice function" is thrown then load the **dplyr** package with `library(dplyr)` and try again.
...
Now, press the `Push` button with the green arrow. Your changes have now been saved to Github! No need to worry about `example_files` right now. Quarto always creates a directory named `filename_files` and uses it for the tools/residues of the renfering process. Nothing in such files is worth saving.
...
```         
tutorial.helpers::show_file("quarto-1.qmd", chunk = TRUE)
```
