...
<!-- DK: Move comments that apply to 2b. -->
...
We want to make the plot in a script that's inside of a separate RStudio project. To do that, we need to get out of the project we're in right now! The way to do that is quite simple, as RStudio has a menu for managing projects:
...
Go ahead and press it, but keep in mind that **doing so will force you to close the tutorial**. This is fine! You can just re-open the tutorial once you're done. 

### Exercise 2

***Do not start this exercise until you have read all the instructions. This exercise will force you to close the tutorial, so you won't be able to access the instructions while completing the exercise.***
...
<!-- DK: Next question should be "Run `getwd()` at the Console." CP/CR. -->

### Exercise 3

Now, we have to create the script where we'll be creating our plot. There are 3 ways of doing that:

<!-- DK: Be consistent. Is it "File -> .." or `File -> ...`. -->

<!-- DK: Is it `Ctrl-Shift-N` or `Ctrl/Cmd + Shift + N` or something else? -->

<!-- DK: Make sure that these concepts have been introduced, or maybe don't use them at all! -->

 1. Through the menu bar on the top of the window. You can press "File -> New File -> R Script";
 2. Through keyboard shortcuts. You can press `Ctrl-Shift-N` (`Cmd-Shift-N` on Mac);
...
<!-- DK: Fix/remove: `Ctrl-S` (`Cmd-S` on Mac).  --> 

file-save is bad. Use `File -> Save`.

Copy-paste the below code into the script you just made. Then, save the file by hitting `Ctrl-S` (`Cmd-S` on Mac) and name the file `script-3.r` (in the file-save menu, you only need to type `script-3`, the `.r` file ending will show up automatically). 
