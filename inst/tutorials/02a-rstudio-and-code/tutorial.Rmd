...
Let's make another plot, this time in a script inside of a separate RStudio project. We'll be making this plot:
...
Your answer should give you the path of the Project you're in right now, `project-1`. 

As a note for clarification: you'll see the word "project" written in multiple ways with multiple meanings:

  1. "project" (capitalized as needed based on grammar). This is the most common meaning of the word. "An individual or collaborative enterprise that is carefully planned to achieve a particular aim." (Oxford English Dictionary)
  2. "**P**roject" (always capitalized). This is a general term for the project-specific workspace made by RStudio that's tied to a *directory* (file folder) on your computer. 
  3. `project` (with `-something` on the end, typically not capitalized). This is the specific term for a Project and it's associated directory. 

### Exercise 2

<!-- DK: Make this simpler. Never close a project without explaining all the steps. -->
<!-- MP: I'm taking another look at this, and can't quite figure out which steps I'm not explaining... -->
...
We want to make the plot in a script that's inside of a separate RStudio project. To do that, we need to get out of the project we're in right now! The way to do that is quite simple, as RStudio has a handy menu for managing projects:

```{r}
include_graphics("images/rs-proj-menu-locator.png")
```

The "Close Project" button is right there in it. 

```{r}
include_graphics("images/rstudio-proj-menu-close-button.png")
```
...
Your answer should indicate that you're now *out* of the `project-1` Project directory. 

### Exercise 3

***Do not start this exercise until you have read all the instructions. This exercise will force you to close the tutorial, so you won't be able to access the instructions while completing the exercise. Once you're done with all the steps in this exercise, re-open the tutorial to continue.***

The next step is to make a new project. We can do that in the same menu by pressing the "New Project..." button instead. You'll be asked to close the tutorial here. Do so, and you'll soon see the New Project Wizard. 
...
Don't forget to open up the tutorial again once you're done. 
...
Your answer should indicate that you're now *in* the `project-2` Project directory. 

### Exercise 5

Now, we have to create the script where we'll be creating our plot. There are 3 ways of doing that:

 1. Through the menu bar on the top of the window. You can press `File -> New File -> R Script`;
 2. Through keyboard shortcuts. You can press `Ctrl/Cmd + Shift + N`;
 3. Through the Files pane. You can navigate to the Project directory and select `New Blank file -> R Script` from the dropdown. 
...
Copy-paste the below code into the script you just made. Then, save the file by hitting `Ctrl-S` (`Cmd-S` on Mac) and name the file `script-3.r`. In the System file-saving dialog (the screen where you pick where a file goes), you only need to type `script-3`, the `.r` file ending will show up automatically. 
...
The output should include the Project file, `project-2.Rproj`, which tells RStudio that the directory it's working from is supposed to be a Project called `project-2`, as well as the file we just made, `script-3.r`. 
