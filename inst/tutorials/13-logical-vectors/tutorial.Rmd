...
```{r boolean-algebra-1-hint, eval = FALSE}
...
Type a code to check if each element of the vector `c(1, 2, NA)` is equal to `NA` using `==`.
...
Start a pipe with `flights`. Pipe it into `group_by`. Within `group_by`, add `year, month, day`. This will group the data in order by year, month, and day.
...
Copy previous code and pipe it into `summarize()`. Within `summarize()`, add `all_delayed = all(dep_delay <= 60)`. This will make a new column named `all_delayed` that is `TRUE` when `dep_delay` is less than or equal to `60` minutes
...
Copy previous code. Within `summarize()`, add `any_long_delay = any(arr_delay >= 300, na.rm = TRUE)`. This will make a new column named `any_long_delay` that is `TRUE` when `arr_delay` is greater than or equal to `300` minutes. It will also remove all `NA` values.
...
This should output 365 rows and 5 columns. The reason there is 365 rows is because the `group_by()` argument groups it by year, month, and day, so all the flights are grouped into their separate days and there are 365 days in a year.

### Exercise 5

Copy previous code. Add `.groups = "drop"` to the `summarize()` function.
...
Copy previous code. Switch `all()` for `mean()`, and `any()` for `sum()`.
...
flights |> 
  group_by(year, month, day) |> 
  summarize(
    all_delayed = ...(dep_delay <= 60, na.rm = TRUE),
    any_long_delay = ...(arr_delay >= 300, na.rm = TRUE),
    .groups = "drop")
```
...
Start a new pipe with `flights`. Pipe it into `filter`. Within `filter()`, add `arr_delay > 0` as the argument. Continue the pipe into `group_by` with the argument `year, month, day`.
...
Continue the pipe with `summarize()`. Within `summarize()`, add the argument `behind = mean(arr_delay)` and `n = n()`. Make sure to separate arguments within a function with comma's.
...
Add `.groups = "drop` into the `summarize()`
