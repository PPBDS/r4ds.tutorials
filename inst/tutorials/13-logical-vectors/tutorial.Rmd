...
Copy previous code. Add `.groups = "drop"` to the `summarize()` function. Also add `.by = year, month`. Make sure to separate these arguments with commas. `.by = year, month` will make the tibble have one row for each variation of `month` and `year`
...
Add `.groups = "drop` into the `summarize()`. Also add `.by = year, month, day` to have one row for each day of the year.
