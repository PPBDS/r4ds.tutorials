...
author: David Kane and Zayan Farooq and Rajarshi Mandal
tutorial:
  id: logical-vectors
output:
  learnr::tutorial:
...
<!-- What is the text below referring to? Fix it. -->

Great work. It's useful to know that this is a shortcut and you can explicitly create the underlying logical variables with `mutate()`

### Exercise 7

<!-- RM: Should I remove numbers or columns in the hint? -->
...
<!-- This has been said earlier. So delete it, -->
...
<!-- A little repetitive. De-repetitify it.  -->
...
<!-- RM: Maybe split this into two parts (description is a little long) -->
...
`final = nov | 12` creates another new variable called `final`. It assigns the Boolean `TRUE` to `final` if `nov` is `TRUE` or if the value `12` is considered as `TRUE`. In R, any non-zero numeric value is considered as `TRUE` in logical operations. Therefore, the expression `nov | 12` performs a logical OR operation between the `nov` column and the value `12`, resulting in a logical vector. The `.keep = "used"` argument keeps only the new variables, and the variables that were used to make them in the tibble.

### Exercise 6

<!-- Explanation of x in y is repeated. Fix! -->

`x %in% y` returns `TRUE` if anything in `x` is in `y`. For example, type `1:12 %in% c(1,5,11)`. This makes `x` a vector containing all the numbers from `1` through `11`, and `y` `1`,`5`, and `11`. Now, click "Run Code"
...
<!-- This has been done a few times now and the learner is probably getting bored. Fix!? -->
...
<!-- This exercise should be added earlier or deleted, as it is repeating what has been stated earlier. Delete away. -->
...
<!-- Not necessarily true. Earlier, we have stated that NA or True is True. Fix. -->

This should output `#> [1] NA NA NA` because when comparing any value (including `NA`) with `NA` using the `==` operator, the result will be `NA`. This is because the value of `NA` represents unknown information, so the result of any comparison involving `NA` is also unknown.
...
<!-- The English here is confused. Fix. -->
...
Copy the previous code and add `any_long_delay = arr_delay >= 300` as a new argument in `summarize()`. Make sure to separate the arguments with commas.
...
<!-- RM: Relation to current exercise -->
...
<!-- Make sure to separate arguments with commas has been written at least five times, maybe delete every other occurence? Good. -->
...
You can also use vectors for the the `TRUE` and `FALSE` arguments. This allows us to create a minimal implementation of `abs()`. Click "Run Code".
...
For example, you could implement a simple version of `coalesce()` as shown below. Click "Run Code".
...
<!-- RM: This section could be more interactive. OK. -->
