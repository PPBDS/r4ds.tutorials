...
This will make the resulting tibble have one row for each variation of `year, month, day`. Basically one row for each day of the year.

### Exercise 6

Make a new pipe with `flights`. Pipe `flights` into `summarize()`. Within `summarize()`, add `all_delayed = mean(dep_delay <= 60, na.rm = TRUE)`. This will create a column called `all_delayed` that is the mean of the rows where `dep_delay` is less than `60`. 
...
This will create a column called `all_delayed` that is the mean of the rows where `dep_delay` is less than `60`

### Exercise 7

Copy precious code and add `any_long_delay = sum(arr_delay >= 300, na.rm = TRUE)`, and `.by = c(year, month)` in the `summarize()` function. Make sure to separate the arguments with commas.
...
Add `.by = year, month, day` to have one row for each day of the year.
