...
If we were to just run `today()`, we would get the date. But, if we use `as_datetime()`, we get the time zone as well, indicating that this is a datetime.
...
<!-- TJ: Need to add some questions with round_date() and ceiling_date() -->
...
```{r modifying-date-time-components-3-ex, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r modifying-date-time-components-3-hint, eval = FALSE}
month(...) <- 01
...
```
...
```{r modifying-date-time-components-4-ex, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r modifying-date-time-components-4-hint, eval = FALSE}
hour(...) <- hour(...) + 1
...
```
...
```{r modifying-date-time-components-5-ex, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r modifying-date-time-components-5-hint, eval = FALSE}
...
```{r modifying-date-time-components-6-ex, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r modifying-date-time-components-6-hint, eval = FALSE}
update(ymd("..."), mday = ...)
```
