...
This tutorial covers material from [Chapter 30 Quarto formats](https://r4ds.hadley.nz/quarto-formats.html) in [*R for Data Science (2e)*](https://r4ds.hadley.nz/quarto.html) by Hadley Wickham, Mine Çetinkaya-Rundel, and Garrett Grolemund. Yet we go into much more detail than that chapter does, especially with regard to the creation and modification of [Quarto websites](https://quarto.org/docs/websites/).
...
Create a Github repo (called "first-website"). Make sure to click the "Add a README file" checkbox. Copy/paste the url for its location at Github.
...
Connect the `first-website` Github repo to an R project on your computer. Name the R project `first-website` also. Keeping the names of repos/projects alligned makes organization simpler. From the Terminal, run `ls`. CP/CR.
...
The project is empty, except for the README file and the project file. We now add all the files necessary for making a website. From the Terminal (not from the Console), type:

`quarto create-project "."  --type website`

Every detail in this command must be correct. The `quarto` command invokes Quarto. The `create-project` phrase tells Quarto that we want to create a project, of which there are several possible types. The `--type website` argument/value at the end tells Quarto that we want a website type of project. The `"."` indicates that we want Quarto to create the project here, in the directory in which we are currently located. A single period `.` is how we indicate the current directory.
...
```
Davids-MBP:first-website dkane$ ls -a
.                       .git                    _quarto.yml             index.qmd
..                      .gitignore              about.qmd               styles.css
.Rproj.user             README.md               first-website.Rproj
Davids-MBP:first-website dkane$ 
```
...
Run `git log` in the terminal. CP/CR.
...
You will then need to hit the Return button twice, accepting the default options for account (your email address) and site name ("first-website"). It should look like this:

```
Davids-MBP:first-website dkane$ quarto publish quarto-pub
? Publish with account: › dave.kane@gmail.com
? Site name: › first-website
...
[✓] Published site: https://davidkane.quarto.pub/first-website
...
A link to your new website should be brought up in a browser.

<!-- DK: Do a lot more here to explain what is going on. Talk about _publish.yaml and _site. -->
...
This tutorial covered material from [Chapter 30 Quarto formats](https://r4ds.hadley.nz/quarto-formats.html) in [*R for Data Science (2e)*](https://r4ds.hadley.nz/quarto.html) by Hadley Wickham, Mine Çetinkaya-Rundel, and Garrett Grolemund. Yet we also went into much more detail than that chapter does, especially with regard to the creation and modification of [Quarto websites](https://quarto.org/docs/websites/).

You should now have a good idea of how to create a Quarto website, and what kind of things you can do with it. 
