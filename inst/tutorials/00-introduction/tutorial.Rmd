...
<!-- What are the steps we often take in looking at a new data set? print it. Look up help for it. These don't require any library be loaded. But the commands below require tidyverse. Run these commands.  -->

<!-- Add glimpse(), summary(), slice_sample(), print() and other commands for looking at a data set. Also drop_na().  DONE-->
...
<!-- Explain to students how to move border pane around.  In the introduction to the setting of RStudio, there could be more information on how to manipulate the size of each pane (ex. to view the content of the tutorial, to have more space for coding). -->
...
<!-- GK: Maybe a bit too long for a knowledge drop -->
...
This is because the variable `x` is only available in the code chunk where it was created. If you want to use `x` in another code chunk, you must assign it again.
...
Print out the first three rows of `diamonds`. You can choose how many rows to display by using the `n` argument in the `print()` function, and how many columns to display by using the `width` argument.
...
This returns the first 3 rows of the `diamonds` data set. The `diamonds` data set is a tibble that contains 53,940 rows and 10 columns. Each row represents a single diamond, and each column represents a different characteristic of the diamond. 
...
Now use `glimpse()` to examine the `diamonds` data set. 
...
Run `summary()` on `diamonds`. This function provides a quick statistics overview of each variable in the data set including number of observations, Min, 1st Qu, Median, Mean, 3rd Qu, and Max. From this you can see the range of values for each. 
...
Run `slice_sample()` on `diamonds` to randomly select 50 rows from the data set using the argument `n = 50`.
...
In later tutorials, we will often work with large data sets. To make it easier to work with these data sets, we will often use the `slice_sample()` function to randomly select a small number of rows. This allows us to quickly see the structure of the data set and to test our code on a smaller data set before running it on the full data set.

### Exercise 12

It's always a good idea to remove missing values from your data set. Use `drop_na()` to remove all rows with missing values from the `diamonds` data set.
...
Having missing values in your data set can cause problems when you try to analyze the data. The `drop_na()` function removes all rows with missing values from the data set.
...
Referring to what pops up when you run `?mpg`, what do `displ` and `hwy` stand for? 
...
It's always necessary to understand the data you are working with. What do the variables represent? What are the units?
...
```{r how-this-book-is-organized-10-hint-2, eval = FALSE}
... +
  labs(title = "...",
       subtitle = "...",
       x = "...",
