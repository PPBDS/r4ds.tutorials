...
<!-- Nothing is returned, which is often the case with R code. But note the check mark which has appeared next to "Exercise 4" above. This indicates that you have submitted your answer. -->
...
```{r unnamed-chunk-2, include = FALSE}
diamonds
```
...
```{r unnamed-chunk-3, include = FALSE}
print(diamonds)
```
...
```{r unnamed-chunk-4, include = FALSE}
print(diamonds, n = 3)
```
...
```{r unnamed-chunk-5, include = FALSE}
glimpse(diamonds)
```
...
```{r unnamed-chunk-6, include = FALSE}
summary(diamonds)
```
...
```{r unnamed-chunk-7, include = FALSE}
glimpse(mpg)
```
...
```{r unnamed-chunk-8, include = FALSE}
ggplot(data = mpg)
```
...
```{r unnamed-chunk-9, include = FALSE}
...
```{r unnamed-chunk-10, include = FALSE}
ggplot(data = mpg, 
       mapping = aes(x = displ, 
                     y = hwy))
```
...
```{r unnamed-chunk-11, include = FALSE}
ggplot(data = mpg, 
       mapping = aes(x = displ,
                     y = hwy)) +
  geom_point()
...
```{r unnamed-chunk-12, include = FALSE}
ggplot(data = mpg, 
       mapping = aes(
         x = displ,
         y = hwy)) +
...
```{r unnamed-chunk-13, include = FALSE}
ggplot(data = mpg, 
       mapping = aes(
         x = displ,
         y = hwy)) +
...
```{r unnamed-chunk-14, include = FALSE}
gss_cat
```
...
```{r unnamed-chunk-15, include = FALSE}
gss_cat |> 
  print()
```
...
```{r unnamed-chunk-16, include = FALSE}
gss_cat |> 
  filter(age > 88)
```
...
```{r unnamed-chunk-17, include = FALSE}
gss_cat |> 
  filter(age > 88) |> 
...
```{r unnamed-chunk-18, include = FALSE}
gss_cat |> 
  filter(age > 88) |> 
  select(age, marital, race, relig, tvhours) |> 
