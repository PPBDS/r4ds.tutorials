...
In the Console, bring up the help page for **nycflights13** by running `help(package = "nycflights13")`. Copy/paste the names and the 2-3 word descriptions of the 5 data sets.
...
Note that, when you hit "Run Code," there were will no output. In general, when you create an object in R, nothing is printed out.
...
```{r rows-20, exercise = TRUE}

```

```{r rows-20-hint-1, eval = FALSE}
flights |>
  count(origin, dest, ... = TRUE)
```

```{r rows-20-test, include = FALSE}
flights |>
