...
<!-- DK: Add drop_na() and formula = y ~ x to make messages and warnings go away in first two plots. -->
...
<!-- DK: Make the warnings go away! -->

<!-- DK: The formatting of the hints is bad/inconsistent. The tests are not consistent with the desired answers. Sloppy! -->
...
```{r creating-first-ggplot-3, exercise = TRUE}
...
```{r creating-first-ggplot-3-test, include = FALSE}
library(ggthemes)
```
...
When we run `glimpse(penguins)`, we see so many records (or observations): a set of measurements made under similar conditions.
...
`penguins`, because it is the start of the pipe, becomes the input which is being plugged into the function `ggplot()`. This generates the same output as `ggplot(data = penguins)`. While indentation may not affect code, it is a good habit to do because it helps go through the code much faster.
...
`geom_point()` is not the only geometric object, there are more than this single one and we will learn about them in other sections in this tutorial.
...
Scatter plots are useful for displaying relationships between `x` and `y` but it is also helpful to ask yourself if there are other variable which might contribute to the relationship. For example, does the relationship between `x` and `y` differ for different species of penguins?
