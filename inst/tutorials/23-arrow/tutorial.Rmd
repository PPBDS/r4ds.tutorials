...
```{r dowload-the-data, include = FALSE, cache=TRUE}
...
<!-- DK: Fix Introduction and Summary. -->
...
The reason that this tutorial took so long to start running is because it was downloading a 9 GB csv file which contains 41,389,465 rows that tell you how many times each book was checked out each month from April 2005 to October 2022 from the Seattle library system. That csv file is named `seattle-library-checkouts.csv` and has been placed in the `data` directory.

<!-- Maybe add an exercise to confirm that the file has downloaded? Not sure that this can work. -->
...
Write `seattle_csv` gets `open_dataset()`. Within `open_dataset()`, add the argument `sources = "data/seattle-library-checkouts.csv"`
...
```{r, include=FALSE}
# Add this back in if it is quick.
# seattle_csv <- open_dataset(
#  sources = "data/seattle-library-checkouts.csv"
# )
```
