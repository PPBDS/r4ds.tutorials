...
<!-- DK: These are some quality questions from the Primer chapter 2. Do they belong here? -->
...
<!-- In both this question and the previous one, we used `list()` to create an object with two elements. To learn more about list columns, read [How To Work With List Columns](https://www.rstudio.com/resources/webinars/how-to-work-with-list-columns/). -->
...
<!-- We can replace the `~ sum(.)` to simply sum as a statement, but we generally work with formulas more often, so we practice by using `~ sum(.)` instead. -->
...
Add `n = n()` to the call to `summarize()`.
...
Add the median of `a` and median of `b`, `med_a = median (a)` and `med_b = median (b)`, to the call to `summarize()`. Remember the arguments to summarize must be separated by commas.
...
The above process is repetitive --- we simply copy-pasted `med_a = median(a)` 4 times and then changed the variable name. This violates our rule of thumb to never copy and paste more than twice.
...
Start a new pipe by sending `df` to `summarize()`. Add `n = n()` to `summarize()`.
...
Add `across(a:d, median)` to the call to `summarize()`. 
...
`across()` makes it easy to apply the same transformation to multiple columns, allowing you to use `select()` semantics inside "data-masking" functions like `summarize()` and `mutate()`.  Note how you get the variable names `a` through `d` "for free," without having to specify them as when using `mutate()`.
...
Add `.by = grp` to the call to `summarize()`
...
Copy the previous code. Add `across(everything(), median)` to the call to `summarize()`. You should have a comma between arguments to summarize.
...
Grouping columns (`grp` here) are not included in `across()`, because they're automatically reserved by `summarize()`. There are 2 selection techniques that are particularly useful for `across()`: `everything()` and `where()`.
...
There is a lot going on here. We have four variables: `a` through `d`. We have two functions: `med` and `n_miss`. The magic of `across()` is that it applies every function to each variable, creating new names for the output columns.
...
Now, add the argument `c(carat, x:z)` to the function `summarize_means()`.
...
Add the argument `everything()` to the function `pivot_longer()`. 
...
Pipe `paths` into the function `map()`.
...
This combination of `map()` and `list_rbind()` allows all the spreadsheets to be viewed at once and combined into one. Like what we did above with `n_max = 1`, you can also customize how many rows you want to see.
...
The names created in the last exercise are automatically carried along by all the `map()` functions, so the list of data frames will have those same names.
...
<!-- AC: gapminder was commented out earlier, so this Exercise doesn't work. -->
...
<!-- AC: It may be good to explain what tolower does. Also I'm getting the following error from exercises 5-10
ℹ In argument: `id = tolower(id)`.
...
Assign the name `process_file` to `function(path)`, followed by a `{}` with the previous code as the body. 
...
Add `process_file` as an argument to `map()`.
...
Recall that this argument `col_name` retrieves/sets column names.
...
<!-- AC: gapminder was commented out, so this Exercise doesn't work either.  -->
...
<!-- AC: Didn't we already make a `files` function?  -->
...
<!-- AC: There seem not to be any failed paths, perhaps due to the fact that `files` is already used? -->
...
Now, let's test it by piping `con` into `tbl("gapminder")`. (We provide the creation code. You add the pipe below it.)
...
Below the provided code, pipe `paths` into `map(append_file)`.
...
```{r writing-csv-files-7-test, eval = FALSE}
?walk2
```

<!-- AC: include = FALSE in the test chunk still runs ?walk2, leading to the help page opening.  -->
...
<!-- AC: list.files() gives `character(0)` -->
...
This function will only work if the tibble which you pass in as the value for the `df` argument includes a column called `carat`. If it doesn't, you will get an error like "object 'carat' not found".
...
<!-- AC: I don't think this works because the by_clarity used in the exercises is made in the setup chunk, so it shouldn't be possible to actually modify it? I'm getting a "Can't recycle `.x` (size 8) to match `.y` (size 0)." error in Exercise 5. -->

Assign the name `by_clarity` to the code above.
