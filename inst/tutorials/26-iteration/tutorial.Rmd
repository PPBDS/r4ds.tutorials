...
<!-- This tutorial is functional, but it needs a lot of work. -->
...
This tutorial covers [Chapter 26: Iteration](https://r4ds.hadley.nz/iteration.html) from [*R for Data Science (2e)*](https://r4ds.hadley.nz/) by Hadley Wickham, Mine Çetinkaya-Rundel, and Garrett Grolemund. We use the [**purrr**](https://purrr.tidyverse.org/) package to learn tools for iteration, repeatedly performing the same action on different objects. Key functions include  [`across()`](https://dplyr.tidyverse.org/reference/across.html),  [`map()`](https://purrr.tidyverse.org/reference/map.html), and user-generated anonymous functions.
...
```{r selecting-columns-with-cols-14-test, include = FALSE}
df_miss |> 
  summarize(across (a:d, 
                     list (median = \(x) median(x, na.rm = TRUE),
                           n_miss = \(x) sum(is.na(x)))),
...
You should see a 1 x 9 table. Now, compare your table to the table produced earlier. What did you notice?
...
a_median a_n_miss b_median b_n_miss c_median c_n_miss d_median d_n_miss
   ...
   0.139        1    -1.11        1   -0.387        2     1.15        0
````

````        
Table from Exercise 25:
A tibble: 1 × 9
...
```{r column-names-10-hint-1, eval = FALSE}
df_miss |> 
  filter(if_all(...))
```
...
<!-- DK: This would be much easier without intermediary objects. Why not just a pipe? Answer seems to be that walk2() is not really designed to work with pipes. -->
...
```{r writing-csv-files-9-ex, exercise = TRUE}

```


```{r writing-csv-files-9-hint, eval = FALSE}
list.files(pattern = ...)
```
