...
<!-- DK: There is something here which makes Format Tutorial Chunk Labels work poorly. Problems manifested themselves in the "Column names" section at Exercise 3. (Exercise 2 works fine, as you can confirm by changing one of the labels and then re-Format'ting.). Presumably the quoted tibble returns cause the issue. Should add this issue as a test case in tutorial.helpers and then fix it here. Problem goes away in later Sections. -->
...
```{r selecting-columns-with-cols-13, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r selecting-columns-with-cols-13-hint-1, eval = FALSE}
df_miss |> 
  summarise(...(a:d, ...),
            n = n())
```

```{r selecting-columns-with-cols-13-test, include = FALSE}
df_miss |> 
...
```{r selecting-columns-with-cols-14, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r selecting-columns-with-cols-14-hint-1, eval = FALSE}
df_miss |> 
...
```{r column-names-3-hint, eval = FALSE}
df_miss |> 
  mutate()
```
...
```{r column-names-4-hint, eval = FALSE}
df_miss |> 
  mutate(across(a:d, 
                \(x) coalesce(x, 0)))
```
...
```{r column-names-6-hint, eval = FALSE}
df_miss |> 
  mutate(
...
```{r column-names-7-hint, eval = FALSE}
df_miss |> 
  filter()
```
...
```{r column-names-8-hint, eval = FALSE}
df_miss |> 
  filter(if_any())
```
...
```{r column-names-9-hint, eval = FALSE}
df_miss |> 
...
```{r column-names-10, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r column-names-10-hint, eval = FALSE}
df_miss |> 
  filter(if_all(...))
```

```{r column-names-10-test, include = FALSE}
df_miss |> 
...
```{r across-in-functions-12, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r across-in-functions-12-hint-1, eval = FALSE}
df_paired |> 
  pivot_longer(...,
               names_to = c("group", ".value"),
               names_sep = "_")
...
```{r across-in-functions-12-test, include = FALSE}
df_paired |> 
  pivot_longer(everything(),
               names_to = c("group", ".value"),
               names_sep = "_")
