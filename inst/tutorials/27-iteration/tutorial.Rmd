...
```{r reading-multiple-fil-35-hint, eval = FALSE}
paths |> 
  map(readxl::read_excel)
```
...
```{r reading-multiple-fil-36-hint, eval = FALSE}
tibble()
```
...
```{r reading-multiple-fil-37-hint, eval = FALSE}
tibble(
    col_name = names(df))
```
...
```{r reading-multiple-fil-38-hint, eval = FALSE}
tibble(
    col_name = names(df), 
    col_type = map_chr(df, vctrs::vec_ptype_full))
```
...
```{r reading-multiple-fil-39-hint, eval = FALSE}
tibble(
    col_name = names(df), 
    col_type = map_chr(df, vctrs::vec_ptype_full),
    n_miss = map_int(df, \(x) sum(is.na(x))))
...
```{r reading-multiple-fil-40-hint, eval = FALSE}
function(df) {
  tibble(
    col_name = names(df), 
    col_type = map_chr(df, vctrs::vec_ptype_full),
...
```{r reading-multiple-fil-41-hint, eval = FALSE}
... <- function(df) {
  tibble(
    col_name = names(df), 
    col_type = map_chr(df, vctrs::vec_ptype_full),
...
```{r reading-multiple-fil-42-hint, eval = FALSE}
df_types(gapminder)
```
...
```{r reading-multiple-fil-43-hint, eval = FALSE}
paths |> 
  map()
```
...
```{r reading-multiple-fil-44-hint, eval = FALSE}
paths |> 
  map(possibly())
```
...
```{r reading-multiple-fil-45-hint, eval = FALSE}
paths |> 
...
```{r reading-multiple-fil-46-hint, eval = FALSE}
... <- paths |> 
  map(possibly(\(path) readxl::read_excel(path), NULL))
```
...
```{r reading-multiple-fil-47-hint, eval = FALSE}
files |> list_rbind()
```
...
```{r reading-multiple-fil-48-hint, eval = FALSE}
...
```{r reading-multiple-fil-49-hint, eval = FALSE}
...
```{r saving-multiple-outp-2-hint, eval = FALSE}
...
```{r saving-multiple-outp-3-hint, eval = FALSE}
...
```{r saving-multiple-outp-4-hint, eval = FALSE}
con |> tbl("gapminder")
```
...
```{r saving-multiple-outp-5-hint, eval = FALSE}
function(path){}
```
...
```{r saving-multiple-outp-6-hint, eval = FALSE}
function(path) {
...
```{r saving-multiple-outp-7-hint, eval = FALSE}
function(path) {
  df <- readxl::read_excel(path)
  df$year <- parse_number(basename(path))
...
```{r saving-multiple-outp-8-hint, eval = FALSE}
...
```{r saving-multiple-outp-9-hint, eval = FALSE}
...
```{r saving-multiple-outp-10-hint, eval = FALSE}
...
```{r saving-multiple-outp-11-hint, eval = FALSE}
con |> 
  tbl("gapminder")
```
...
```{r saving-multiple-outp-12-hint, eval = FALSE}
con |> 
  ... |> 
  count(year)
```
