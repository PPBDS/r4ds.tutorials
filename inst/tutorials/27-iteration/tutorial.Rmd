...
<!-- Don't use the same variable name for the input and output columns! Use `med_b` for the median of the variable `b`. DONE.- MB -->
...
Now, I want to look at data from year 2020, 2021 and 2022. What would you do to achieve it?

You can simply copy and past the previous code and change the years respectively.

### Exercise 2

Now, copy and past the previous code 4 times, change `"data/y2019.xlsx"` to `"data/y2020.xlsx"`, `"data/y2021.xlsx"` and `"data/y2022.xlsx"`.
...
```{r reading-multiple-fil-2-hint, eval = FALSE}
data2019 <- readxl::read_excel("data/y2019.xlsx")
data2020 <- readxl::read_excel("data/y2020.xlsx")
data2021 <- readxl::read_excel("data/y....xlsx")
data2022 <- readxl::read_excel("data/y....xlsx")
```
...
```{r reading-multiple-fil-3-hint, eval = FALSE}
data <- bind_rows()
```
...
```{r reading-multiple-fil-4-hint, eval = FALSE}
...
This is the code from lasgt exercise: `paths <- list.files("data/gapminder", pattern = "[.]xlsx$", full.names = TRUE)`
...
Based on the 12 paths created from the last exercise, call `read_excel()` 12 times to get 12 data frames. Hit `Run Code`.
...
```{r reading-multiple-fil-6-hint, eval = FALSE}
