...
<!-- DK: There are a couple of open-ended questions early in the tutorial. Do they work? -->

<!-- DK: Clean up babynames example. -->

<!-- DK: We need a lot of knowledge drops. And some questions split up into two or three steps. -->

<!-- DK: Add some of the images from the chapter. -->
...
How exactly can we tidy a dataset that is this wide? The majority of the width is from the 76 columns that tell us the rank of the song on a given week in the year 2000. Add `pivot_longer()` to the pipe.
...
Now, we need to specify what `pivot_longer()` needs to do with the new `name` and `value` columns.
...
We do this by continuing our pipe with `mutate()`. Inside of `mutate()`, we can set `week` equal to `parse_number(week)`. This will get rid of all the `wk`'s, replacing them with numbers.
...
```{r lengthening-data-11-test, include = FALSE}
billboard_longer |> 
  ggplot(aes(x = week, 
             y = rank, 
             group = track)) +
...
```{r widening-data-7-test, include = FALSE}
...
```{r widening-data-8-test, include = FALSE}
...
```{r widening-data-9-test, include = FALSE}
...
Let's finish this tutorial by creating an interesting graphic. 
