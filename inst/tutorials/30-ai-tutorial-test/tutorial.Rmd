...
Using the same AI, ask it to generate code for creating an insight of your plot. Here is the prompt: using `str_flatten()` on the babynames dataset, make another unique insight related to the plot we have already made. Insert your plot code if needed. CP/CR
...
Make a new code cell, using AI, ask it to generate code that pipes **babynames** to `count()` with `str_length()` to find the distribution of lengths of US babynames. Run the code to get a new subset of data and print it out. CP/CR
...
Next, get code that pipes **babynames** to `mutate()` and use `str_sub()` to find the first and last letter of each name. Run the code to get a new subset of data and print it out. CP/CR
