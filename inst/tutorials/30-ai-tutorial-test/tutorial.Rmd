...
Now, we are going to create a new title using string functions. Using AI, ask it to generate code for creating the title of your plot using the the function str_c() and assign it to the variable title1. The title should stay the same, but they way it is made should be through str_c().
...
The function str_c() takes any number of vectors as arguments and returns a character vector. It essentially combines arguments of the same type into a string. Add this code to your code chunk before the graph, and within the call to labs() in your graph, set title = "title1".
...
The function str_glue() is an alternative to str_c() that evaluates anything inside {} like it’s outside of the quotes. However, it currently converts missing values to the string "NA."


### Exercise 14

Using the same AI, ask it to generate code for creating an insight of your plot. Here is the prompt: using str_flatten() on the babynames dataset, make another unique insight related to the plot we have already made. Insert your plot code if needed.

```{r ai-usage-14}
question_text(NULL,
    message = "Here is our generated code:\n# Step 1: Filter names starting with \"A\" in 1950
top_a_1950 <- babynames %>%
  filter(year == 1950, str_starts(name, \"A\")) %>%
  arrange(desc(prop)) %>%
  slice_head(n = 5)

# Step 2: Use str_flatten to create a readable insight
insight_text <- top_a_1950 %>%
  pull(name) %>%
  str_flatten(collapse = \", \", last = \" and \")

# Step 3: Create final sentence
insight <- str_c(
...
Using the same AI, ask it to generate code that will find the most common second letters of babynames starting with "A" with the function separate_longer_delim().
...
Try runninig our code! Isolate the part with the relavant function (if needed), and use it to print out the changed dataset to see what happened.
...
Using the same AI, ask it to generate similar code that will find the most common second letters of babynames starting with "A" with the function separate_longer_position().
...
Using the same AI, ask it to generate code that will combine two columns using str_c(), and then separate them again with separate_wider_delim().
...
question_text(NULL,
  ...
  count(sex_clean, name_clean, sort = TRUE)",          
    answer(NULL, 
           correct = TRUE),
    allow_retry = TRUE,
    incorrect = NULL,
...
Try runninig our code! Isolate the part with the relavant function (if needed), and use it to print out the changed dataset to see what happened. For this one, print out the change after the first mutate, and then print out what happens after the separate_wider_delim().
...
Using the same AI, ask it to generate code that will combine two columns using str_c(), and then separate them again with separate_wider_position().
...
Try runninig our code! Isolate the part with the relavant function (if needed), and use it to print out the changed dataset to see what happened. For this one, print out the change after the first mutate, and then print out what happens after the separate_wider_position().
...
## Using String Functions on Babynames
###
