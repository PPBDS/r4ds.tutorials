...
Let's do the same thing on single quote, so create a new variable `y` and set it to `"'"` using `<-`. Then on a new line, type `str_view()` and set `y` as argument.
...
What if I have too many backslashes and quotes to include that I get confused by the amount of backslashes and quotes. To demonstrate the problem, create a variable `tricky` and set it to `"double_quote <- \"\\\"\" # or '\"' single_quote <- '\\'' # or \"'\""`
...
Copy the previous code and type `str_view()` and set `tricky` as the argument.
...
When we run it we should get `double_quote <- "\"" # or '"' single_quote <- '\'' # or "'"` but to make a string representation like this is very confusing sicne you need a lot of backslashes and quotes. In the next exercise we will see how we solve the problem.

### Exercise 13

Copy the previous code and modify `tricky` to be  
`r"(double_quote <- "\"" # or '"' single_quote <- '\'' # or "'")"` and run it with `str_view()`.

Now the string has much less quotes and backslashes but will we get same result?
...
To eliminate the problem we used a raw string:  a special type of string literal that allows you to represent strings without any special character interpretation or escape sequences.
...
Let's now see how `str_view()` handeles special characters.

Create a new variable `x1` and set it equal to vector which contains:`"one\ntwo", "one\ttwo", "\u00b5", "\U0001f604"`
...
All we get is the same as what we types and dosent actually take all the special characters that we put except the smiley face.
...
Start a pipe with `df3` to `group_by()` which takes `name` as argument.
...
```{r creating-many-string-15-hint-1, eval = FALSE}
... |>
  ...(name) |>
  ...()
```
...
```{r creating-many-string-16-hint-1, eval = FALSE}
... |>
  ...|>
  summarize(fruits = ...)
...
CP/PR and start pip with `df2` to `separate_longer_position()`, within the function, type the column `x` and then set the `width` to 1.
...
CP/PR and start pip with `df3` to `separate_wider_delim()`, within the function, type `x`, then set the `delim` to "." and finally set the names to a vector which looks like ` c("code", "edition", "year")`.
...
CP/PR and start a pipe with `df4` to `separate_wider_position()`, within the function, type `x`, then set the `widths` to  a vector which looks like ` c(year = 4, age = 2, state = 2)`.
...
CP/PR and start pipe with `df` to `separate_wider_delim()`, within the function, type `x`, then set the `delim` to "-" and finally set the names to a vector which looks like `c("x", "y", "z")`.
...
CP/PR and within `separate_wider_delim()` add `too_few` to be "debug". Also create a variable `debug` and set it to the pipe then at the end call `debug`.
...
CP the previous code and change `too_few` to be set to "align_start".
...
```{r extracting-data-from-13-hint, eval = FALSE}
df <- tibble(x = c(...))
... |> 
  separate_wider_delim(
    x,
...
```{r extracting-data-from-14-hint, eval = FALSE}
df <- tibble(x = ...)
```
...
```{r extracting-data-from-15-hint, eval = FALSE}
df <- tibble(x = ...)
... |> 
  separate_wider_delim(
    ...,
...
CP the previous code add `too_many = "debug"` within `separate_wider_delim()`.
...
```{r extracting-data-from-16-hint, eval = FALSE}
df <- tibble(x = ...)
... |> 
  separate_wider_delim(
    ...,
...
CP the previous code and change `too_many` to "drop".
...
```{r extracting-data-from-17-hint, eval = FALSE}
df <- tibble(x = ...)
... |> 
  separate_wider_delim(
    ...,
...
As we already based of the word, Rstudio will drop the x remainders.
...
```{r extracting-data-from-18-hint, eval = FALSE}
df <- tibble(x = ...)
... |> 
  separate_wider_delim(
    ...,
...
Type `str_sub()` and within the argument, for the first argument type `y` which we declared, then 1 which is starting position and then 3 our ending position.
...
To see what `charToRaw()` insert the string "hadley" into the function.

```{r nonenglish-text-1, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r nonenglish-text-1-hint-1, eval = FALSE}
charToRaw(...)
```
...
In the past, there were multiple encoding standards for non-English characters. For Europe alone, there were Latin1 (ISO-8859-1) for Western European languages and Latin2 (ISO-8859-2) for Central European languages. Thankfully, we now have a widely supported standard called UTF-8. UTF-8 can encode almost all human characters, including emojis.

To see how the encoding differs, let's first create a variable `x2` and set it equal to `"text\nEl Ni\xf1o was particularly bad this year"`.

```{r nonenglish-text-2, exercise = TRUE}

```

```{r nonenglish-text-2-hint-1, eval = FALSE}
x2 <- ...
```
...
Type `read_csv()` and insert x2 as the argument.

```{r nonenglish-text-3, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r nonenglish-text-3-hint-1, eval = FALSE}
read.csv(...)
```

### 

You get something like this "El Ni\xf1o was particularly bad this year" when you run this. 

readr uses UTF-8 everywhere. This is a good default but will fail for data produced by older systems that don’t use UTF-8. If this happens, your strings will look weird when you print them. Sometimes just one or two characters might be messed up like we saw.

### Exercise 4

To fix the string, copy the previous code and add `locale = locale(encoding = "Latin1")`.

```{r nonenglish-text-4, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r nonenglish-text-4-hint-1, eval = FALSE}

```

### 

We should get "El Niño was particularly bad this year" because we changed the encoding to be Latin1 which will find use of special characters which we added.

### Exercise 5

To demonstarte another encoding, create a variable `x3` and set it to `"text\n\x82\xb1\x82\xf1\x82\xc9\x82\xbf\x82\xcd"`.

```{r nonenglish-text-5, exercise = TRUE}

```

```{r nonenglish-text-5-hint-1, eval = FALSE}
x3 <- ...
```

### 

The number's and backslashes represent special characters which are supposed to be encoded in Japanese.


### Exercise 6

Run `x3` within `read_csv()`.

```{r nonenglish-text-6, exercise = TRUE}

```

```{r nonenglish-text-6-hint-1, eval = FALSE}
read_csv(...)
```

### 

When you run it all you get it the numbers and backslashes that we typed. In the next exercises we will specify the encoding via the `locale` arguement.

If you want to learn more about reading excel files or spreadsheets, check out the [R for data science](https://r4ds.hadley.nz/spreadsheets.html).

### Exercise 7

Let's now modify the previous code and add locale = locale(encoding = "Shift-JIS")

```{r nonenglish-text-7, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r nonenglish-text-7-hint-1, eval = FALSE}
...
```{r nonenglish-text-8, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r nonenglish-text-8-hint-1, eval = FALSE}
...
```{r nonenglish-text-9, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r nonenglish-text-9-hint-1, eval = FALSE}
...
```{r nonenglish-text-10, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r nonenglish-text-10-hint-1, eval = FALSE}
...
```{r nonenglish-text-11, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r nonenglish-text-11-hint-1, eval = FALSE}
