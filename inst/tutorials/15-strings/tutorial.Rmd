...
```{r creating-a-string-5-hint, eval = FALSE}
x <- "n"
x
```
...
The reason we don't see n only withouth quotes is that R shows the whole string as a vector.
...
```{r creating-a-string-6-hint, eval = FALSE}
library(...)
```
...
```{r creating-a-string-7-hint, eval = FALSE}
str_view(...)
```
...
<!-- DK: Next exercise is way too hard. Needs four preliminary questions first. Stuff like: -->
...
```{r creating-many-string-1-hint, eval = FALSE}
str_c(...,...)
```
...
```{r creating-many-string-2-hint, eval = FALSE}
