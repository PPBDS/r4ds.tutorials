...
<!-- AC: Technically the 01 in rescale 01 is to scale between 0 and 1, so rescale02 is not really describing the function. -->
...
<!-- AC: Also, it may be nice to also try out the new rescale02() in an Exercise (with the same examples as rescale01) -->
...
<!-- AC: It may be nice to also explain what the code does.  -->
...
<!-- AC: It may be better to just give a preview of df1 first (whether in an exercise or just in a paragraph). -->
...
```{r data-frame-functions-4-hint-1, eval = TRUE}
