...
Create a new pipeline. Pipe flights to the `mutate()` function. Within the `mutate()` function, create a new variable `carriers` and set it to `n_distinct(carrier)`.
...
Using your previous code, within your call to `mutate()` add a the `.by` argument and set it to `dest`.
...
flights |> 
  mutate(carriers =  n_distinct(carrier), .by = ...)
```

```{r counts-6-test, include = FALSE}
flights |> 
  mutate(carriers = n_distinct(carrier), .by = dest)
```

### 

Setting `.by` to `dest` lets us group by the destination.


### Exercise 7

Continue the pipe to `select()`, with the columns `dest` and `carriers` as arguments.
...
flights |> 
  mutate(carriers = n_distinct(carrier), .by = dest) |>
  select(dest, carriers)
```

### 

This selects the column `dest` from our original data and our newly-created `carriers` column.

### Exercise 8

Continuing the pipe, add the `arrange()` function to your pipeline. Within the function `arrange()` add the argument of `carriers`. 
...
Using your previous code, within your call to `arrange()`, use the argument `desc(carriers)` so that it arranges the carriers in descending order.
...
Now, count the number of cancelled flights or missing values of each destination. With a new pipeline, pipe `flights` to the `mutate()` function. Within the call to `mutate()`, create a new variable called `n_cancelled` and set it equal to `sum(is.na(dep_time)`.
...
flights |> 
  mutate(n_cancelled = ...(is.na(dep_...)))
```

```{r counts-12-test, include = FALSE}
flights |> 
  mutate(n_cancelled = sum(is.na(dep_time)))
```
...
Within your call to the `mutate()` function, add the `.by` argument and set it to `dest`.

```{r counts-13, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r counts-13-hint-1, eval = FALSE}
flights |> 
  mutate(n_cancelled = sum(is.na(dep_time)), .by = ...)
```

```{r counts-13-test, include = FALSE}
flights |> 
  mutate(n_cancelled = sum(is.na(dep_time)), .by = dest)
```

### Exercise 14

Continue the pipe to `select()`, with the columns `dest` and `n_cancelled` as our arguments.

```{r counts-14, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r counts-14-hint-1, eval = FALSE}
... |>
  select(..., n_cancelled)
```

```{r counts-14-test, include = FALSE}
flights |> 
  mutate(n_cancelled = sum(is.na(dep_time)), .by = dest) |>
  select(dest, n_cancelled)
```
...
```{r counts-15, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r counts-15-hint-1, eval = FALSE}
... |>
  distinct()
```

```{r counts-15-test, include = FALSE}
flights |> 
  mutate(n_cancelled = sum(is.na(dep_time)), .by = dest) |>
  select(dest, n_cancelled) |>
  distinct()
...
The job of the `distinct()` function is to remove any duplicates, so every entry is unique and not duplicated. Two models ago, we used the function `n_distinct()`, which had the `distinct()` function built-in to it.


### Exercise 16

Let's finalize our model by piping it to `arrange()`, with the arguments 

```{r counts-16, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r counts-16-hint-1, eval = FALSE}

```

```{r counts-16-test, include = FALSE}

```

### 

Our final model now shows us the number of cancelled flights in each airport, and in descending order. It's good to know that Chicago O'Hare (ORD) had the highest number of cancelled flights!
