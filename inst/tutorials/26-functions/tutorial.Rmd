...
```{r vector-functions-7-hint, eval = FALSE}
df |> ...(
  a = rescale01(a),
  b = rescale01(...),
  ... = rescale01(c),
...
```{r vector-functions-8-hint, eval = FALSE}
...
```{r data-frame-functions-9-hint, eval = FALSE}
diamonds |> summary6(...)
```
...
```{r data-frame-functions-10-hint, eval = FALSE}
diamonds |>
  ...(cut)|>
  summary6(...)
```
...
```{r data-frame-functions-11-hint, eval = FALSE}
...
```{r data-frame-functions-12-hint, eval = FALSE}
...
```{r data-frame-functions-13-hint, eval = FALSE}
dimaonds |>
  count_prop(...)
```
...
```{r data-frame-functions-14-hint, eval = FALSE}
