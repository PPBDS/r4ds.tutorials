...
Copy the code and after `geom_point()`, add the `geom_smooth()` layer 
...
```{r plot-functions-10, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r plot-functions-10-hint-1, eval = FALSE}
sorted_bars <- function(..., var) {
  ... |> 
    mutate({{ ... }} := fct_rev(...({{ var }}))) |>
    ggplot(aes(... = {{var}}))+
...
```{r plot-functions-16, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r plot-functions-16-hint-1, eval = FALSE}
... |>
  ...(carat, ...) +
  labs(x = ..., y = ...)
```
