...
<!-- Biggest hackery is our use of the "file =" trick to bring in code examples which would mess up the formatting of this document (and, even worse, mess up the use of format_tutorial()) if we just included them raw, like with ```` or ````markdown or ````{verbatim}. The trick works in that nothing breaks. But it is annoying! We wold much rather see the code which we are including **right there** instead of having to look it up in a separate file. -->
...
To "render" our Quarto document, click the "Render" arrow at the top of the Code pane, or use the shortcut `Command/Ctrl + Shift + K`.
