...
<!-- Will we ever move from Rpubs to Quarto Pub? Maybe. The help page for the quarto package does not look useful, but ChatGPT assures me that this is possible with quarto_render() and quarto_publish(). But, maybe not . . . I think that, for now, you need to issue a terminal command to put a Quarto document on Rpubs. -->
...
```{r script-2-11-hint-1, eval = FALSE, echo = TRUE}
library(tidyverse)
...
```{r unnamed-chunk-150, file = "images/quarto-example.txt", echo = TRUE, eval = FALSE}
```
...
```{r unnamed-chunk-15, file = "images/chunk-example.txt", echo = TRUE, eval = FALSE}
```
...
```{r unnamed-chunk-151, file = "images/empty-chunk-example.txt", echo = TRUE, eval = FALSE}
```
...
```{r unnamed-chunk-152, file = "images/label-chunk-example.txt", echo = TRUE, eval = FALSE}
```
...
```{r unnamed-chunk-155, echo = FALSE}
scat_p
```
...
```{r unnamed-chunk-156, file = "images/inline-example-1.txt", echo = TRUE, eval = FALSE}
```
...
```{r unnamed-chunk-160, file = "images/inline-example-2.txt", echo = TRUE, eval = FALSE}
```
...
We'll be publishing our Quarto documents on the Internet by using [Posit Cloud](https://posit.cloud/), a free web publishing service. Posit Cloud is managed by [Posit](https://posit.co/) (formerly RStudio), a leading company in the data science world. You will need to create a free account. Do not give them your credit card number.


### Exercise 1

<!-- DK: Would be better if the question could ask for a link to some aspect of the student's account. -->

Sign up for a [free Posit Cloud](https://posit.cloud/plans/free) account. Once you are logged in, you will see a "What's New" menu item on the left side. Click on it. Look at the item. Choose one. Copy/paste the url for that item.
...
````
https://posit.cloud/learn/whats-new#introducing-posit-recipes
````

It doesn't which item you chose. We just want to ensure that you were able to create an account.


### Exercise 2

Render the `quarto-4.qmd` file. By rendering, you are generating an html file, which is what Posit Cloud publishes on the web.
...
```{r publishing-3, out.width = "875px"}
