...
<!-- Biggest annoyance comes in Quarto 1 section when we display the example Quarto document. See the discussion there. In essence, don't want format_tutorial() till you, temporarily, cut the four backticks back to three.  -->
...
<!-- Mention: https://rstudio.github.io/visual-markdown-editing/options.html in the context of setting global options, like avoiding Visual. -->
...
Again, there is no necessary connection between the **title** of a Quarto document and the **name** of the file.

Note the several different ways in which we use the word "Source." First, there is there "Source" pane, the 1/4 of the RStudio workspace which displays you code for editing. Second, there is the "Source" button, the think to click when you want to source your R scripts. Third, you can edit a Quarto document in either "Source" or "Visual" mode.
...
If RStudio continues to insert `editor: visual` in the YAML header of new documents, you can try to edit the option by hand: 
...
Delete everything in `quarto-2.qmd` except for the YAML header. Do not delete the dashes which provide the border for the YAML header. The file should look like:
...
The key lesson is that we are operating in two worlds simultaneously: QMD World and Console World. We are responsible for keeping them in sync with tools like `Cmd/Cntrl + Enter`.
...
<!-- DK: I think that this section could be improved. Maybe show all the steps together, like we do with creating a project? -->
