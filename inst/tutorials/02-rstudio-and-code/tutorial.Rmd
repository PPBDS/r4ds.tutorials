...
<!-- Standardize the use of commands like cat(tail(readLines("quarto-1.qmd")), sep = "\n") and explain them to the student early in the tutorial. -->
...
If this returns `TRUE`, then you have a "typical" computer set up, with a Desktop in your home directory. (The tilda symbol, `~`, refers to a user's home directory.) We recommend that you store your RStudio projects there. However, more experienced computer users should feel free to ignore this advice. If the retur value is `FALSE`, you will need to create a location by hand.
...
Run `cat(readLines("analysis-1.Rproj"), sep = "\n")` in the R Console. CP/CR.

Note what this code is doing. First, `readLines()` reads in all the text in `analysis-1.Rproj` as a vector. We could just print this out, but that would be ugly. So, we use `cat()` to print out that vector, with each element separated by a newline character: `\n`. 
...
Run `cat(readLines("quarto-1.qmd", n = 2), sep = "\n")` in the Console. CP/CR.
...
`readLines()` reads in each line in a text file into a character vector. The argument `n = 2` restricts that to just the first two lines. `cat()` prints out the lines. The `sep = "\n"` argument places a new line character between each element of the vector returned by `readlines()`.
...
Run `cat(tail(readLines("quarto-1.qmd")), sep = "\n")` in the Console. CP/CR.
...
<!-- DK: Explain the use of tail() in the command. -->
...
Delete everything except the YAML header, which is the text in the box at the top of the file, including the dashes. Add `This is my first Markdown text.` two lines below the YAML header.

### 

Save the file and run `cat(readLines("quarto-1.qmd"), sep = "\n")`. CP/CR.
...
Render the document. It is good to get in the practice of using the short-cut key `Command/Ctrl + Shift + K` instead of the Render button. Cool people use short-cut keys. They don't press buttons.
...
Run `cat(readLines("quarto-1.qmd"), sep = "\n")` in the Console. CP/CR.
...
Render the document, just to make sure it worked. Note that rendering automatically causes the document to be saved. To encourage the use of short-cut keys going forward, we will start using them directly in our instructions to you. So, instead of writing "Render the document," we will just write `Command/Ctrl + Shift + K`. 
...
Run `cat(readLines("quarto-1.qmd", n = 6), sep = "\n")` in the Console. CP/CR
...
Save the file, and run `cat(tail(readLines("quarto-2.qmd")), sep = "\n")` in the Console. CP/CR.
...
Run `cat(tail(readLines("quarto-2.qmd")), sep = "\n")` in the Console. CP/CR.
...
Below the code chunk, add the following sentence to the QMD: `The value of x is ?, a surprisingly high value.` `Command/Ctrl + Shift + K`.
...
Include the backticks. They tell Quarto that you want to evaluate the code `x` using the R lanaguge. In this case, that code is just the letter `x`, which evaluates to (i.e., prints out) the number we have assigned to it. `Command/Ctrl + Shift + K`.
...
`Command/Ctrl + Shift + K`. You might notice that, strictly speaking, you do not need to save the document before you render it. With the usual default settings, RStudio will automatically save a document which you try to render.
...
Our HTML file looks ugly, both because of the annoying message and because it shows the R code. We want neither. Add `echo: false` and `message: false` to the code chunk options. `Command/Ctrl + Shift + K`. Copy/paste the HTML, which should include nothing but a title and author.
...
`Command/Ctrl + Shift + K`. Run `readLines("quarto-4.qmd") |> tail(6)`. CP/CR.
