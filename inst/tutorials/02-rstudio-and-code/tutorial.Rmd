...
<!-- RPubs works well, just as it always has! It works from the publish button, which is nice for the initial learning. It produces a nice URL, which you can edit. The main page of your account is a nice portfolio of work. The only reason to move away is a fear that Rpubs will be going away. -->
...
<!-- Quarto Pub is not available from the Publish button. Also, although quarto_publish_site() seems to work as advertized, and allows you to publish a full website, I can't get quarto_publish_doc() to work at all. Seems like Quarto from the command line would work fine for individual docs: quarto publish quarto-pub document.qmd. But we don't want to use the Terminal before the Terminal tutorial, at least too much. -->
 ...
 <!-- At first, my Quarto document was rendered and previewed in Windows instead of the viewer - could add instructions on how to switch to preview in the viewer pane. Explain the settings (gear) button and how we can change it. Maybe default should be the Viewer tab, especially given how much copy/pasting the students need to do. -->
...
<!-- does all the necessary magic, including creating the new directory would do the trick. But I dislike this option because it creates a useful R directory in the project, which can only confuse things. I am also suspicious of the `.Rproj.user` directory which is created.  -->
