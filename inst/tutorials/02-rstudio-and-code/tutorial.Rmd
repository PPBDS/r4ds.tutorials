...
<!-- If we ever change the default settings so that Source with Echo is the default, we will need to revisit the String 1 instructions. -->

<!-- Will we ever move from Rpubs to Quarto Pub? Maybe. The help page for the quarto package does not look useful, but ChatGPT assures me that this is possible with quarto_render() and quarto_publish(). But, maybe not . . . I think that, for now, you need to issue a terminal command to put something on Rpubs. -->
...
<!-- Things from chapter 3 and 5 that might be discussed/quoted: -->

<!-- 1. snake_case; functions(),   -->

<!-- 2. Finally, be wary of writing very long pipes, say longer than 10-15 lines. Try to break them up into smaller sub-tasks, giving each task an informative name. The names will help cue the reader into what’s happening and makes it easier to check that intermediate results are as expected. -->

<!-- 3. As your scripts get longer, you can use sectioning comments to break up your file into manageable pieces -->
...
This function returns the "path" to your R working directory. A path is simply the information which allows you to reach a location on your computer. In this case, if you wanted to find the folder from which R is running, you would follow the path returned by `getwd()`.
...
The path to this folder should not include any folders with spaces (or other weird characters) in their names. For example, this is a reasonable location for R projects:
...
Now that you have read all the directions, we can go through the steps. But note that, when you first click `File -> New Project`, you will get the "Terminate Jobs" warning. (Recall the discussion from the "Getting Started" tutorial.)
...
Type `5 * 5` into your R script file. Save the file. Name it `script-1.R`. (Note that R will provide the `.R` suffix, so you only need to typr `script-1`.) 
...
R scripts include permanent copies of your code. You can save them and also work with them interactively.

### Exercise 2

Note the "Source" button, which is in the top right corner of the Code Editor pane. Note the little black upside down triangle next to "Source".  If you click on it you will see two choices: "Source" and "Source with Echo". Click on "Source". See what happens in the Console. CP/CR.
...
````
> source("~/Desktop/projects/analysis-1/script-1.R")
> 
````

### 

Whenever you run an R script file by hitting the "Source" button, Rstudio will send all the code in the file to the R Console. Since our code does not call, directly, for anything to be printed, nothing happens.

### Exercise 3

There are often "short-cut key" combinations which perform the same task as clicking a button. Placing your cursor inside the R script window and hit or using the keyboard shortcut `Cmd/Ctrl + Shift + S` is the same as hitting the "Source" button.

The short-cut key for "Source with Echo" is `Cmd/Ctrl + Shift + Enter`. Try it now. CP/CR.
...
Clicking buttons or typing short-cut keys does the same thing under the hood: they cause specific R commands to be issued. In this course, we cause RStudio to run the `source()` function, as before, but with the `echo=TRUE` option. This causes RStudio to run the code, show the code and show the results of the code.

Using short-cut keys is quicker and more professional than clicking buttons.

### Exercise 4

In your R script, type `6 + 3` in the line after `5 * 5`. Save the file with `Cmd/Cntrl + s`. "Source with Echo" this file. CP/CR.
...
Every line in the file is executed. Each line is also echoed. Finally, the results of each line, if there are any, is printed.
...
Go back to the first line in your R script. Change `5 * 5` to `x <- 5 * 5`, thereby creating an object named `x` with a value of 25. Save the file. Click the "Source" button.
...
Click `File -> New File -> R Script`. At the top of the Code Editor Pane, the file should be called `Untitled1`. You will see that the text is in red, with an asterisk. This indicates that there are unsaved changes in the file.
...
Click the save icon below the script title, or use the shortcut `Cmd/Ctrl + s` to save the script. Let's call this file `script-2`. The `.R` suffix is added automatically.
...
If you have already loaded the **tidyverse** package in this session, and have not restarted R, then you will see no output because RStudio has already provided you with this message previously.
...
````
Warning message:
In readLines("script-2.R") : incomplete final line found on 'script-2.R'
````
...
Save the Quarto document with `File -> Save`. You can also click the disk icon toward the upper right of the Code Editor pane or by use the shortcut `Cmd/Ctrl + s`. Name the file `quarto-1`. The `.qmd` file extension is added automatically.
...
Note that there is no necessary connection between the **title** of a Quarto document and the **name** of the file.


### Exercise 3

In the top left corner above your Quarto document, you should see the buttons "Source", and "Visual". "Source" should be selected. If it is not, select it. 
...
<!-- DK: Replace the png with the actual text. -->
...
Delete everything in `quarto-2.qmd` except for the YAML header. Delete the `editor: visual` line from within the YAML header. Do not delete the dashes which provide the border for the YAML header. The file should look like:
...
The `editor: visual` line tells RStudio to switch to "Visual" mode, rather than "Source" mode, when you first open the file. Most people prefer to work in Source mode, which is the default.

If you uncheck the "Editor: Use visual markdown editor" box at the bottom of the "New Quarto Document" box, then the `editor: visual` line will not appear in the initial YAML header.


### Exercise 3

Add a code chunk either by pressing the Add Chunk command at the top of the editor on the right (the green C button with a plus sign next to it) or by using the shortcut key combination: `Cmd + Option + i` / `Ctrl + Alt + i`.

The newly created code chunk will be empty, looking like:

<!-- DK: Very annoying that this use of verbatim messes up the formating of the rest of the file, both greying out text and hiding the Outline. But changing to using ```` just makes the R code chunk disappear. -->
...
Right now, the **tidyverse** package has not been loaded into the current R session. To load it, you must run the code chunk. This can be done using the Run button in the top right of the editor, or with the shortcut `Cmd/Ctrl + Shift + Enter`.
...
Because we have not added any code chunk options, we see the code in the html file, along with the title and author. But there is no output because assignment of a value does not result in output in R.
...
The short-cut key to restart R is `Cmd/Cntrl - Shift - 0`. You should restart your R session regularly. Indeed, one of our catch phrases is:
...
The key lesson is that we are operating in two worlds simultaneously: QMD World and Console World. We are responsible for keeping them in sync with tools like `Cmd/Cntrl - Return`.
...
Make a new header with the title "Diamonds Histogram". Remember we do this using `##`, followed by a space. Skip a line under this header, and then create a new code chunk. Use the code chunk argument `echo: false` to stop your code from showing up when you render the file.
...
We'll be publishing our Quarto documents on the Internet by using [Rpubs](https://rpubs.com/), a free web publishing service. Rpubs is managed by [Posit](https://posit.co/) (formerly RStudio), a leading company in the data science world. You will need to create an Rpubs account.
