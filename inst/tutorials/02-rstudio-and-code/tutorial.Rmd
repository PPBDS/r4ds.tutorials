...
<!-- Biggest hackery is our use of the "file =" trick to bring in code examples which would mess up the formatting of this document (and, even worse, mess up the use of format_tutorial()) if we just included them raw, like with ```` or ````mardown or ````{verbatim}. The trick works in that nothing breaks. But it is annoying! We wold much rather see the code which we are including **right there** instead of having to look it up in a separate file. -->
...
<!-- The "file =" trick works well, but I sure which there were a way to just include text directly without this hackery. It is a pain to keep track of a bunch of separate text files. -->

```{r, file = "quarto-example.txt", echo = TRUE, eval = FALSE}
```
...
```{r, file = "chunk-example.txt", echo = TRUE, eval = FALSE}
```
...
```{r, file = "empty-chunk-example.txt", echo = TRUE, eval = FALSE}
```
...
```{r, file = "label-chunk-example.txt", echo = TRUE, eval = FALSE}
```
...
```{r, file = "inline-example-1.txt", echo = TRUE, eval = FALSE}
```
...
```{r, file = "inline-example-2.txt", echo = TRUE, eval = FALSE}
```
