...
<!-- Should include a better code chunk options example. Not sure warning message example discussion works. Worth improving. Or maybe replacing with something simpler like echo or eval. Just want students to be comfortable with code chunk options.  -->
...
<!-- DK: Add some more questions to this section? Discuss the .Rproj contents, perhaps. -->

Good data scientists keep their work organized. You should place your work in different R Projects, which are simply folders (or directories) on your computer with some helper files. Each separate R Project is a folder within your main folder for your R work, i.e.,  "/Users/dkane/Desktop/projects/"
...
Whenever you run an R script file by hitting the "Source" button, Rstudio will send all the code in the file to the R Console, as if you had retyped it all there and hit return.
...
<!-- DK: Used to discuss the use of `Cmd/Ctrl-Shift-S`. Not sure if this is used enough to justify. Also, this seems to, by default, Source with no echo, which is confusing. Even worse, doing this seems to change the default behavior of the Source button from sourcing with echo to sourcing without. Too confusing for this stage is student education! -->
...
`ls()` returns a list of objects present in your environment. These are the same objects displayed in your Environment tab in the top right corner of RStudio. You should see that it returned `x`, since you defined `x` using your R script.
...
Click the save icon below the script title, or use the shortcut `Cmd/Ctrl + s` to save the script. The first time you save a file you will also name it. Let's call this file `script-1`. The `.R` suffix is added automatically.
...
Create a new R script file with `File -> New File -> R Script`. Name it `script-2.R`. 
...
Scripts and projects give you a solid workflow that will serve you well in the future. Aways remember to create one RStudio project for each data analysis project. That project may contain multiple scripts files, along with other material.
...
`ggplot()` will generate a blank plot, at least until a `geom` layer is added. Note that you might get a warning when using `readLines()` if `script-2.R` does not end with a blank line. This warning does not matter to us but it is always good practice to end a text file with a blank line.
...
Now we have the code which creates a plot in `script-2`. How do we use this plot for other things? Right now we have to run `script-2` every time we want to see the plot, which is a little inconvenient.
