...
<!-- Only edit the .gitignore one time, for each website? --><!-- Figure out what is happening at the start of the Adding a plot section wrt background jobs. Not sure what this refers to. -->

<!-- Should Website 3 be different? Start from the IDE and makes a new directory with git box checked, and then use `usethis::use_github()` to connect to Github. It is useful to see this work, even if it is not our default strategy. Or should we just make sure students understand one approach well? If so, we could explain this approach in a knowledge drop. -->
...
<!-- SU: I think generally it will ask you if you want to use rstudio or to not open it. If you have vscode installed and running on your laptop, vscode should be an option as well. -->
