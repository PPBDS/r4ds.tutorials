...
<!-- DK: Add test chunks to each question, except ones which are tested as part of making one plot. -->
...
```{r, include=FALSE}
flights |> 
  summarize(hour = sched_dep_time %/% 100, prop_cancelled =  
              mean(is.na(dep_time)))
```
...
flights |> 
  summarize(hour = sched_dep_time %/% 100, prop_cancelled =  
              mean(is.na(dep_time)), n = n(), .by = hour)
```
...
Using the same pipe as above, delete the `.by` argument. Add a new argument, within the call to `summarize()`, called `.groups` and set it to `"drop"`. 
...
The `.groups` argument controls the grouping structure of the output. The groups can be kept ("keep") or dropped ("drop").
...
Using the same pipe as above, delete the `.by` argument. Add an argument, within the call to `summarize()`, called `.groups` and set it to `"drop"`. 
