...
Create a new pipeline. Pipe flights to the `summarize()` function. Within the `summarize()` function, create a new variable `carriers` and set it to `n_distinct(carriers)`.
...
Using your previous code, within your call to `summarize()` add a the `.by` argument and set it to `dest` so that we can group by the destination.
...
Now, count the number of cancelled flights or missing values of each destination. Create a new pipeline. Pipe `flights` to the `summarize()` function. Within the call to `summarize()`, create a new variable called `n_cancelled` and set it to `sum(is.na(dep_time)`.
...
Using the same pipe as above, within your call to the `summarize()` function, add an argument called `.by` and set it to `dest`.
...
flights |> 
  summarize(hour = ..., prop_cancelled = ..., n = ...)
```
...
flights |> 
  summarize(hour = ..., prop_cancelled = ..., n = ..., .by = ...)
```
...
<!-- DK: This is sloppy. We are using two concepts at the same time. First, is the specific calculation using the `x7` vector. The second, in the knowledge drop, is the generic calculation which works for any vector `x`. Students find this juxtaposition confusing! -->

`x - lag(x)` gives you the difference between the current and previous value for all the elements of the vector `x`.

### Exercise 17

Type in `x7 == lag(x7)` and hit "Run Code".
...
Pipe `repetition` to the `summarize()` function. Within the call to `summarize()`, create a variable `id` and set it to `consecutive_id(x)`.
...
Using your previous code, within your call to the `summarize()` function, add the `.by` argument and set it to `id` so that we can group by `id`.
...
Create a new pipeline. Pipe `flights` to the `summarize` function. Within the call to `summarize()`, add the `.by` argument and set it to `c(year, month, day)`.
