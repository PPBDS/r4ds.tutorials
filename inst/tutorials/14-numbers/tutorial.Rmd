...
Using the same pipe as above, add the `mutate()` function to the pipeline. Within the `mutate()` function, create a new variable `r` and set it to `min_rank(sched_dep_time)`.
...
Using the same pipe as above, add the `filter()` function to the pipeline. Within the `filter()` function, add the argument `r %in% c(1, max(r))`.
