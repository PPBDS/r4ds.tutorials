...
<!-- DK: Are you sure that all the components, like parisdate, which students need to work with are, in fact, created in the setup chunk? -->

<!-- DK: Be careful about section names that are identical in the first 20 characters.  -->

<!-- DK: Using arguement names like `time` and `datetime` is a bad idea, I think. -->
...
```{r creating-datetimes-t-2-hint-1, eval = FALSE}
...()
```
...
```{r creating-dates-and-t-1, exercise = TRUE}

```

```{r creating-dates-and-t-1-hint-1, eval = FALSE}
glimpse(...)
```
...
```{r creating-dates-and-t-2, exercise = TRUE}

```

```{r creating-dates-and-t-2-hint-1, eval = FALSE}
flights |>
...
```{r creating-dates-and-t-3, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r creating-dates-and-t-3-hint-1, eval = FALSE}
... |>
    mutate(...,
           sched_dep_time = ...))
```
...
```{r creating-dates-and-t-4, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r creating-dates-and-t-4-hint-1, eval = FALSE}
... |>
    mutate(...,
           sched_dep_time = ...)) |>
    select(dep_delay, dep_time, sched_dep_time)
...
```{r creating-dates-and-t-5, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r creating-dates-and-t-5-hint-1, eval = FALSE}
... |>
    ggplot(aes(x = ..., 
               y = ...))
```
...
```{r creating-dates-and-t-6, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r creating-dates-and-t-6-hint-1, eval = FALSE}
... |>
  ... +
  geom_point()
```
...
```{r creating-dates-and-t-7, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r creating-dates-and-t-7-hint-1, eval = FALSE}
... |>
  select(...) |>
    ...()
```
...
```{r creating-dates-and-t-8, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r creating-dates-and-t-8-hint-1, eval = FALSE}
ggplot(aes(x = ..., 
           y = ...,
           color = ...))
```
...
```{r creating-dates-and-t-9, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r creating-dates-and-t-9-hint-1, eval = FALSE}
+ labs(title = ...,
                subtitle = ...,
                color = "Flight",
                x = ...,
...
```{r datetime-components--1, exercise = TRUE}

```

```{r datetime-components--1-hint-1, eval = FALSE}
glimpse(...)
```
...
<!-- DK: Not a good idea to have a variable with the same name as a function. Please fix. -->

Let's start a pipe with the `flights_dt` to `mutate()` our data. Set `wday` to `wday(dep_time, label = TRUE)`.

```{r datetime-components--2, exercise = TRUE}

```

```{r datetime-components--2-hint-1, eval = FALSE}
... |> 
  mutate(wday = ...)
```
...
```{r datetime-components--3-hint-1, eval = FALSE}
... |> 
 ... |> 
  ggplot(aes(x = ...))
```
...
<!-- DK: Don't use rdrr.io as a reference. -->
<!-- DK: Use backticks around functions names. -->
...
```{r time-spans-with-dura-1, exercise = TRUE}

```

```{r time-spans-with-dura-1-hint-1, eval = FALSE}
... <- today() - ymd("2000-8-30")
```
...
```{r time-spans-with-dura-2-hint-1, eval = FALSE}
age
```
...
```{r time-spans-with-dura-3-hint-1, eval = FALSE}
as.duration(...)
```

### 

<!-- DK: This is not clear to me. -->
...
```{r time-spans-with-dura-4-hint-1, eval = FALSE}
... * 2 + dweeks(...)
```
...
```{r time-spans-with-peri-1-hint-1, eval = FALSE}
time <- ymd_hms("2026-03-08 01:00:00", tz = "America/New_York")
time
```
...
```{r time-spans-with-peri-2-hint-1, eval = FALSE}
time + ...
```
...
```{r time-spans-with-peri-3-hint-1, eval = FALSE}
time + ... * ... - ...
```
...
```{r time-spans-with-peri-4-hint-1, eval = FALSE}
... + dyears(...)
... + years(...)
```
...
```{r time-spans-with-inte-1-hint-1, eval = FALSE}
... <- ymd("2021-01-01")
```
...
```{r time-spans-with-inte-2-hint-1, eval = FALSE}
date2020 <- ...("2020-01-01")
```
...
```{r time-spans-with-inte-3-hint-1, eval = FALSE}
... <- date2020 %--% date2021
```
...
```{r time-spans-with-inte-4-hint-1, eval = FALSE}
... / days(1)
years(1) / ...
```
