...
```{r time-spans-dura-1, exercise = TRUE}

```

```{r time-spans-dura-1-hint, eval = FALSE}
... <- today() - ymd("2000-8-30")
```
...
```{r time-spans-with-dura-2, exercise = TRUE}

```

```{r time-spans-with-dura-2-hint, eval = FALSE}
age
```
...
```{r time-spans-with-dura-3, exercise = TRUE}

```

```{r time-spans-with-dura-3-hint, eval = FALSE}
as.duration(...)
```

###

With `as.duration()` we are able to get the time difference in seconds and years through the help of the lubridate alternative. Durations always record the time span in seconds.

### Exercise 4

Multiply the `as.duration()` from above by `2` and add `dweeks(12)`.

```{r time-spans-with-dura-4, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r time-spans-with-dura-4-hint, eval = FALSE}
... * 2 + dweeks(...)
```
...
Set `ymd_hms("2026-03-08 01:00:00", tz = "America/New_York")` to the `time` variable and run the variable to see the datetime.

```{r time-spans-with-peri-1, exercise = TRUE}

```

```{r time-spans-with-peri-1-hint, eval = FALSE}
time <- ymd_hms("2026-03-08 01:00:00", tz = "America/New_York")
time
```
...
Let's add `dweeks(3)` to `time`.

```{r time-spans-with-peri-2, exercise = TRUE}

```

```{r time-spans-with-peri-2-hint, eval = FALSE}
time + ...
```
...
Let's add 10 to `time`, multiply by `hours(20)` and subtract `months(5)`.

```{r time-spans-with-peri-3, exercise = TRUE}

```


```{r time-spans-with-peri-3-hint, eval = FALSE}
time + ... * ... - ...
```
...
Add `dyears(2)` to `time`. On a separate line add `years(2)` to `time`.

```{r time-spans-with-peri-4, exercise = TRUE}

```

```{r time-spans-with-peri-4-hint, eval = FALSE}
... + dyears(...)
... + years(...)
```
...
```{r time-spans-with-inte-1, exercise = TRUE}

```

```{r time-spans-with-inte-1-hint, eval = FALSE}
... <- ymd("2021-01-01")
```
...
```{r time-spans-with-inte-2, exercise = TRUE}

```

```{r time-spans-with-inte-2-hint, eval = FALSE}
date2020 <- ...("2020-01-01")
```
...
```{r time-spans-with-inte-3, exercise = TRUE}

```

```{r time-spans-with-inte-3-hint, eval = FALSE}
... <- date2020 %--% date2021
```
...
```{r time-spans-with-inte-4, exercise = TRUE}

```

```{r time-spans-with-inte-4-hint, eval = FALSE}
... / days(1)
years(1) / ...
```
