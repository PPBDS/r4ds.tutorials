...
```{r time-zones-1-hint, eval = FALSE}
...()
```
...
```{r time-zones-3-hint, eval = FALSE}
... <- ymd_hms("2023-06-20 18:00:00", tz = ...)
```

### Exercise 4

Subtract `newdate` from `parisdate`.

```{r time-zones-4, exercise = TRUE}

```

```{r time-zones-4-hint, eval = FALSE}
parisdate - ...
```
...
We can combine datetimes, as well with different timezones. Set `combined` to `c()`. Within `c()` include `parisdate` and `newdate`. Run `combined` after to see the result.

```{r time-zones-5, exercise = TRUE}

```

```{r time-zones-5-hint, eval = FALSE}
combined <- c(..., ...)
```
...
```{r time-zones-6-hint, eval = FALSE}
...(combined, tzone = ...)
```
...
```{r time-zones-7-hint, eval = FALSE}
