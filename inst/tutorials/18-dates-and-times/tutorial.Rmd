...
Run `today()` and `now()` in the code chunk below. With this we can get the current date or date-time.

```{r creating-data-times-im-1, exercise = TRUE}

```

```{r creating-data-times-1-im-hint, eval = FALSE}
...()
...()
```
...
```{r creating-data-times-2-im, exercise = TRUE}

```

```{r creating-data-times-2-im-hint, eval = FALSE}
... <- "date
  01/02/15"
```
...
Let's read this data by using the `read_csv()` function on the `csv` from above.

```{r creating-data-times-3-im, exercise = TRUE}

```

```{r creating-data-times-3-im-hint, eval = FALSE}
...(csv)
```

###

With `read_csv()` we are able to analyze the aspects of the date-time with `col_types` such as `col_date()`, and `col_datetime()`. We can view the year features, month features, day features, time features, and other features.

### Exercise 4

Now we are going to utilize `col_types` to format date from the `csv` we read. Set `col_types` to cols(date = col_date("%m/%d/%y")) in `read_csv()``.

```{r creating-data-times-4-im, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r creating-data-times-4-im-hint, eval = FALSE}
...
Let's try this now with a different format. Set `col_types` to cols(date = col_date("%d/%m/%y")) in `read_csv()`.

```{r creating-data-times-5-im, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r creating-data-times-5-im-hint, eval = FALSE}
read_csv(csv, col_types = ...)
```
...
With non-English dates and %b or %B you will need to use `locale()`. Check out [**date_names_langs()**](https://readr.tidyverse.org/reference/date_names.html) or create with `date_names()`.
...
Set `ymd()` to "2017-01-31". On another line set `mdy()` to "January 31st, 2017".

```{r creating-data-times-strings-1, exercise = TRUE}

```

```{r creating-data-times-strings-1-hint, eval = FALSE}
ymd(...)
...("January 31st, 2017")
```
...
Using these three formats we need to identify the order in which year, month, and day appear in your dates, then arrange “y”, “m”, and “d” in the same order. That gives you the name of the lubridate function that will parse your date.

###

### Exercise 2

Besides this we can also force set this by supplying a timezone. Set `tz` within to `ymd()` to "UTC".

```{r creating-data-times-2, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r creating-data-times-2-hint, eval = FALSE}
ymd("2017-01-31", tz = ...)
```
...
```{r creating-data-times-strings-3, exercise = TRUE}
make_datetime_100 <- function(year, month, day, time) {
  make_datetime(year, month, day, time %/% 100, time %% 100)
}
```

```{r creating-data-times-strings-3-hint, eval = FALSE}
ymd_hms(...)
```

###

With this we are able to create a datetime that is automatically set to UTC. `hms()` stands for **h**our, **m**inute, **s**econd.
...
flight_grph <- flights |>
            ...
            ggplot(mapping = aes(x = sched_dep_time, y = dep_time, color=flight)) +              geom_point() + 
            labs(title = "Flights",
...
View the `flights` data through `glimpse()`

```{r creating-datetimes-w-1, exercise = TRUE}

```

```{r creating-datetimes-w-1-hint, eval = FALSE}
glimpse(...())
```

###
Here we can see all of the `flights` data with all of the important aspects of the `flights` data.

### Exercise 2

We want to look at the `dep_time` and the `sched_dep_time`. Mutate the `flights` data by setting `dep_time` and `sched_dep_time` to `make_datetime()` and selection the columns of `year`, `month`, `day`, and `dep_time/sched_dep_time`.
...
flights |>
  mutate(dep_time = make_datetime(..., ..., ..., dep_time),
            sched_dep_time = make_datetime(..., ..., ..., sched_dep_time))
```

###
Here we are able to make a datetime based on the `flights` data that we have. When creating a a date/time from this sort of input, we use `make_date()` for dates, or `make_datetime()` for date-times.

### Exercise 3

Continue the extension by adding `ggplot()` and in `aes()` set `x` to `sched_dep_time` and `y` to `dep_time`
...
```{r creating-datetimes-w-3-hint, eval = FALSE}
... |>
...
Let's plot this by adding `geom_point()` and within `aes()` set `color` to `flight`
...
```{r creating-datetimes-w-4-hint, eval = FALSE}
... |>
...
```{r creating-datetimes-w-5-hint, eval = FALSE}
+ labs(title = ...,
                subtitle = ...,
                color = "Flight",
                x = ...,
...
```{r creating-datetimes-w-end, eval = FALSE}
flight_grph
```

###
Besides the use of datetime, we can use date which can be done through `as_datetime()` and `as_date()`. Visit [**as_datetime()**](https://lubridate.tidyverse.org/reference/as_date.html) for more information.
...
###
We are going to use the `flights_dt` dataset which includes the `dep_time`, `sched_dep_time`, `arr_time`, `sched_arr_time` with datetimes to see our data.

### Exercise 1

Let's `glimpse()` the `flights_dt` data to take a look at the data found within `flights_dt`

```{r datetime-components-getting-1, exercise = TRUE}

```

```{r datetime-components-getting-1-hint, eval = FALSE}
glimpse(...)
```

###
You will find that there are 9 main columns which contain all of the information that we will use regarding the information about our flights.

### Exercise 2

Let's start an extension with the `flights_dt` to mutate our data with `wday()` set to `wday(dep_time, label = TRUE)`.

```{r datetime-components-getting-2, exercise = TRUE}

```

```{r datetime-components-getting-2-hint, eval = FALSE}
flights_dt |> 
  mutate(wday = wday(..., label = ...))
```
...
```{r datetime-components-getting-3-hint, eval = FALSE}
... |> 
 ... |> 
  ggplot(aes(x = ...))
```
...
Let's view our data with `geom_bar()`. Add the bar to the data that we have used.
...
```{r datetime-components-getting-4-hint, eval = FALSE}
... |> 
  ... |> 
  ... +
  geom_bar()
...
Let's use the `flights_dt` dataset again by `mutate()`. Set `dep_hour` to `dep_time`- `floor_date(dep_time, "day").

```{r datetime-components-rounding-1, exercise = TRUE}

```

```{r datetime-components-rounding-1-hint, eval = FALSE}
flights_dt |>
  mutate(dep_hour = ... - ...())
```
...
Let's plot the data through `ggplot()` Set `x` to `dep_hour` in `aes()`.
...
```{r datetime-components-rounding-2-hint, eval = FALSE}
... |>
  ... |>
  ggplot(aes(x = ...))
```
...
Let's use `geom_freqpoly()` to view the difference by setting `binwidth` to "60*30" in `geom_freqpoly()`.
...
```{r datetime-components-rounding-3-hint, eval = FALSE}
... |>
  ... |>
  ... +
...
Let's modify this date: "2026-07-08 12:34:56" with the `ymd_hms()` function.
...
```{r datetime-components-modifying-1-hint, eval = FALSE}
(datetime <- ymd_hms("..."))
```
...
```{r datetime-components-modifying-2-hint, eval = FALSE}
year(...) <- 2030
datetime
```
