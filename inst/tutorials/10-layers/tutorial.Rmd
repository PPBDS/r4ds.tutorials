...
```{r aesthetic-mappings-advanced-10, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r aesthetic-mappings-advanced-10-hint-1, eval = FALSE}
... +
  labs(...,
       color = "Clarity",
       ...)
...
Use `ggplot()` to map `age` to the x-axis.
...
Note: no points will be plotted because `geom` has not been used

### Exercise 5

Add the layer `geom_density()`.
...
`geom_density()` is a plotting function that creates a smooth density estimate of a variable.

### Exercise 6

Set `fill` to `party` within the `aes()` function in `ggplot()`. 
...
Within `geom_density()` set the `alpha` argument to 0.5. 
...
`geom_density()` can be combined with other geoms and aesthetics to create complex visualizations.
...
With `xlim()` we can specify the range of values that will be displayed on the x-axis, excluding any data points outside that range.
...
You can completely transform the look of your plot by changing its geom, and different geoms can reveal different features of your data. The density graph that we plotted in this example is different from the features presented in the box plot and histogram graph.
...
Currently, we can't tell the difference between parties. We will fix this by changing the colors to represent each party.
...
ggplotly(nominate_plot, tooltip = ...)
