...
<!-- quarto create-project --type website --title "Hello"-->
...
We need to ensure that the latest version of Quarto and of the R **quarto** package are installed.
...
Unfortunately, even the development version of the **quarto** package has some problems. So, we will generally interact with quarto directly from the Terminal.
...
<!-- DK: Don't really need the quarto package, do we? Maybe remove it completely? -->


### Exercise 4

In the Terminal, run `quarto create-project --type website --title "My First Website"`. 
...
```{r rendering-and-publishing-ex-1}
question_text(NULL,
	answer(NULL, correct = TRUE),
	allow_retry = TRUE,
	try_again_button = "Edit Answer",
...
```{r rendering-and-publishing-ex-2}
question_text(NULL,
	answer(NULL, correct = TRUE),
	allow_retry = TRUE,
	try_again_button = "Edit Answer",
...
<!-- DK: Tried to use various versions of quarto_publish_site(server = "https://quartopub.com") but couldn't get any of them to work, so still need to use the command line. Very annoying! -->
...
Copy/paste the url for your website. It should look like:

````
https://davidkane.quarto.pub/first-website
````


```{r rendering-and-publishing-ex-3}
question_text(NULL,
	answer(NULL, correct = TRUE),
	allow_retry = TRUE,
	try_again_button = "Edit Answer",
