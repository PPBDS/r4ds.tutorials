...
<!-- Now that we have an organized/reasonable home page, we can add some other pages which do other things. Example is some of the cool interactive graphics. -->
...
This tutorial covers material from [Chapter 28 Quarto](https://r4ds.hadley.nz/quarto.html) and [Chapter 29 Quarto formats](https://r4ds.hadley.nz/quarto-formats.html) in [*R for Data Science (2e)*](https://r4ds.hadley.nz/quarto.html) by Hadley Wickham, Mine Çetinkaya-Rundel, and Garrett Grolemund. Yet we will go into much more detail, especially with regard to the creation and modification of [Quarto websites](https://quarto.org/docs/websites/). We will learn about Quarto commands like `quarto create`, `quarto render`, `quarto preview` and `quarto publish`, all of which are run from the Terminal.

## Set up environment
### 

<!-- Until quarto 1.4 and the updated quarto R package are in production, we need to install them by hand. But that is OK! By this point in their education, students should be comfortable in in doing these sorts of installations. (Actually, we don't need the updated quarto package, at least in this version.) -->
...
Create a new code chunk in `index.qmd` and load two libraries: **tidyverse** and **ggthemes**. Render the document by pressing the Render button at the top of the Code pane. If your website is still being previewed, this will update the displayed homepage automatically.
...
Note that the term `setup` is a special code chunk label. It should only be used in the chunk which loads libraries and otherwise **setup**s the analysis.
...
Assigning labels to code chunks helps keeps you organized and helps others understand your code. You cannot assign the same label to multiple code chunks. We often use dashes in labels, as in `diamonds-plot` since doing so makes for easier reading.
...
First, we have the qmd files in the project directory which are used to create the website. This is the original source, the places where changes should be made. 

Second, we have the rendered version of the files which reside in the `_site` directory. We do not edit these files directly, but we do look at them when we are "previewing" the website.

Third, we have the website itself, the files which other people can see. These reside on Quarto Pub, in this case, but we could put them at a different hosting service.
...
Italicize the title of your project by replacing the quotation marks with a single asterisk. Example:
...
<!-- DK: Deleted question about lists. -->
...
As you can see. The quote and its source have ended up on the same line because Quarto assumes that anything without an explicit line break can be placed in to the same paragraph.
...
Push all the changes to Git, and issue the `quarto publish` command in the Terminal. You need to hit Enter at least once.
...
A website, when you first look closely, may look overwhelming in its complexity. But it is usually created from of a simple collection of text files, each of them quite simple.
...
After pushing, type `quarto publish` in the Terminal and enter "Yes".


## Using an RDS file
### 

An RDS file allows us to save any R object in a file. When we read from an RDS, we avoid having to run the code required to make the object. Instead, we simply have access to it. This is very convenient for any code which take a long time to run, such as intensive data manipulation.

Restart your R session with `Command/Ctrl + Shift + 0`.

### Exercise 1

Go to `File -> New File -> R Script`. Press save in the left of the top panel. Name the file `process_data.R`. The `.R` suffix will be added automatically.
...
Make sure that `make_plot.R` is one of these files. We often use scripts to to record the exact steps we used in creating a graphic. We want our work to be reproducible. We will often need to perform the same work again, albeit with some changes. A script can make that much simpler.
...
<!-- DK: Once students look at this plot on the web, they will notice that the labels are too small. Show how to use theme() to make the labs elements bigger for the web. -->
