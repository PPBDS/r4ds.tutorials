...
<!-- There is an "annoying" bug discussed above. Should provide a reprex of this to RStudio folks. -->
...
You will need to "Terminate Jobs" this tutorial. And you will be placed into the new project after RStudio restarts, as usual. Just restart the tutorial once this happens.
...
You would get the same set of files if, in the Terminal, you ran `ls -a`. The `-a` ensures that **a**ll the files are listed, included the "hidden" ones which begin with a dot. Recall the `.` means the current directory while `..` means the directory one level above the current one.
...
In the Termina, run `ls`. The "hidden" files will not appear because we did not include the `-a` option to `ls`. CP/CR.
...
We don't want to put all the files on Github. So, add `*Rproj` to your `.gitignore` file. Save the file. 
...
Note that a new line, `/.quarto/`, had already been added to the `.gitignore`. This happened when we ran `quarto create`.  The leading `/` escapes the `.`. In other words, we need the leading `/` if we want to ignore the files in a dotted directory.

In fact, this directory does not yet exist. But the `quarto` command wanted to ensure that, once it does, Git would ignore the contents.
...
<!-- DK: Why isn't `about.html` mentioned? Because the website shows `index.html` by default, so, there is no need to create it? But can't I see that it has been created? I am confused. Is this a bug in Quarto? -->
...
From the Console, run `cat .gitignore`. CP/CR.
...
This command is convenient because the `_sites` directory contains of lot of weird files that change often enough to annoy. We don't create any of them, so we don't really need to back them up. Moreover, they will still get published directly to the web from our computer.
...
```{r rendering-and-previewing-9-ex}
question_text(NULL,
	answer(NULL, correct = TRUE),
	allow_retry = TRUE,
	try_again_button = "Edit Answer",
...
<!-- DK: These reason these aren't in a four backticks area is because format_tutorial() would, for unknown reasons, add a space between # and | if we did that, and such a space makes these no longer function as chunk options when pastes into the students' index.qmd. -->
