...
In the Source pane, open the `_quarto.yml` file. Replace `title: "."` with `title: "A New Website"`. Save the file. Make sure that the indentation is the same as before `title` should be indented to the same extent as `navbar` below it.
...
<!-- DK: Not sure if this step is really necessary, but it is not bad to get students used to closing RStudio and then opening on old project. Indeed, we ought to make them practice such skills! -->

First, however, you should restart RStudio, not just the current R instance. The reason for this is that we want to ensure a clean environment after our prior usage of various Quarto commands. After your restart RStudio, make sure you are within the `first-website` R project. (If, on restart, you are not automagically placed in `first-website`, you can [switch to this project via the project dropdown menu](https://docs.posit.co/ide/user/ide/guide/code/projects.html#opening-projects) in the far upper right-hand corner of RStudio.)

### Exercise 1

Delete everything in `index.qmd` except for the `title: "My First Website"` surrounded by the three dashes, above and below. 
...
````
show_file("index.qmd", chunk = TRUE)
````
...
<!-- This page probably appeared in your Viewer tab in the Output pane even though `quarto render` generally  -->
...
`Command/Ctrl + Shift + K`. You will see a nice plot, along with a lot of junk code and messages. Note that RStudio might only redirect you to the plot the first time you render `index.qmd`. Going forward, you may need to affirmatively click on the correct tab on your browser or on the Viewer tab in the Output pane to see the newly rendered page.
...
Render the `index.qmd` with the "Render" button. It should show the plot. As always, you should render with `Command/Ctrl + Shift + K`. Shortcut keys are lit!
...
Let's create text which is also a hyperlink to the a website at the bottom of `about.qmd`. Enclose the text "Kane's Free Data Science Bootcamp" in brackets `[]`. Then, without a space intervening, add parentheses `()` which enclose a link to the relevant website: `https://bootcamp.davidkane.info`.
...
<!-- Could explain this better, maybe illustrate some step-by-step examples. Maybe in another tutorial, like Chapter 29? -->
