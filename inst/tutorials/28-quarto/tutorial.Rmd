...
<!-- Central problem with this tutorial, as with other document/project creation exercises, is that it is unclear what approach is best for students. There are three choices: point/clock from RStudio, quarto() R package and quarto from the command line. -->
...
<!-- Need to cover other material in these chapters, not just websites. -->

<!-- Make clear whether or not students are doing this in the current RStudio instance or in a new one. -->

<!-- Explain the `/.quarto/` addition to .gitignore. -->

<!-- Show how to use theme() to make the labs elements bigger for the web. -->


<!-- Revisit the use of names like car_plot or map-london.png. I think that, both in the Console and the Terminal, things like the dash in map-london will be evaluated as math, and so generate an error. Perhaps underscores cause errors as well. Either way, we don't need this extra complexity here. -->
...
We won't discuss the `search.json` and `styles.css` files. The `site_libs` directory includes many files beyond the scope of this tutorial. Basic idea is that the entire `_site` directory is a functioning website, ready to be delpoyed.
...
Check your Git pane. Assuming you committed everything before running `quarto publish quarto-pub`, as instructed, it should be easy to notice the new files. Write down the name of the new YAML file, the file with `.yml` at the end.
...
If you look closely, you will see a new directory, `.quarto`, in the Files pane. Neither this directory, nor any of the files within it, appear in the Git pane because of the `/.quarto/` line in the `.gitignore` file.
...
But, if you look at the Quarto Pub version of your website, it still uses the "A New Website" title because we have not (yet) published our new version. We have, however, changed the files in the `_site` directory, as you can tell by looking at the Git pane.
...
The `_publish.yml` file is like the `first-website.Rproj` file. They might contain private information of some sort, like logins or passwords, and so do not belong on Github.
...
Note both the simplicity *and* the complexity of your project. It is simple in that, even if our computer blew up and  
...
Let's add a visualization to your website by modifying `index.qmd`. Recall that this file will always create our "home" page because it is rendered to `index.html` *and* browsers, by default, show `index.html`, if one exists, when they visit a directory. 
...
Delete everything in `index.qmd` except for the `title: "first-website"` surrounded by the three dashes, above and below. Run `head index.qmd` in the Terminal. CP/CR.
...
Create a new code chunk in `index.qmd` and load two libraries: **tidyverse** and **ggthemes**.

```{r adding-visualizations-ex-2, exercise = TRUE}

```

```{r adding-visualizations-2-hint-1, eval = FALSE}
library(tidyverse)
library(ggthemes)
```
...
```{r adding-visualizations-ex-3, exercise = TRUE}

```

```{r adding-visualizations-3-hint-1, eval = FALSE}
mtcars
```

```{r adding-visualizations-3-test, include = FALSE}

```

### Exercise 4

Select the columns `mpg` and `wt` using `select()`.

```{r adding-visualizations-ex-4, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r adding-visualizations-4-hint-1, eval = FALSE}
mtcars |> 
  select(..., ...)
```

```{r adding-visualizations-4-test, include = FALSE}
...
```{r adding-visualizations-ex-5, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r adding-visualizations-5-hint-1, eval = FALSE}
mtcars |> 
  select(mpg, wt) |> 
  ggplot(aes(wt, mpg)) +
  geom_point()
...
```{r adding-visualizations-5-test, include = FALSE}
...
and `se` to `FALSE` in the arguments.

```{r adding-visualizations-ex-6, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r adding-visualizations-6-hint-1, eval = FALSE}
mtcars |> 
  select(mpg, wt) |> 
  ggplot(aes(wt, mpg)) +
  geom_point() +
...
```{r adding-visualizations-6-test, include = FALSE}

```

### Exercise 7

Add `theme_clean()` to slightly alter the feel of the graph.

```{r adding-visualizations-ex-7, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r adding-visualizations-7-hint-1, eval = FALSE}
mtcars |> 
  select(mpg, wt) |> 
  ggplot(aes(wt, mpg)) +
  geom_point() +
...
```{r adding-visualizations-7-test, include = FALSE}

```

### Exercise 8

Use `labs` to add an appropriate title, subtitle, axes labels, and caption to your graph based on the example plot in exercise 1.

```{r adding-visualizations-ex-8, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r adding-visualizations-8-hint-1, eval = FALSE}
mtcars |> 
  select(mpg, wt) |> 
  ggplot(aes(wt, mpg)) +
  geom_point() +
...
```{r adding-visualizations-8-test, include = FALSE}
...
Add these code chunk options to the top of the first code chunk:
...
<!-- MR: this picture includes sidebar:interactive but that is not added until a later section -->
...
Add the function `read_rds()`. Set the value of the `file` argument to the name of the file you want to read, `car-plot.rds`. Assign the output of the call to `read_rds()` to an R object called `car_plot`. 
...
On a new line, type the object `car_plot` to print the plot saved in the .rds. Render the .qmd.
...
In the `car-plot` code chunk, use the function `ggsave()` to make a PNG image of the plot you just made. 

### 

The first argument is the name of the file you want to save the image to, `"car-plot.png"`. 

### 

The next argument is `plot`, set that equal to `car_plot`. 

### 

Render the file. You should see a file named `car-plot.png` appear in your files pane.

### 

In the Terminal, run `ls *png`. CP/CR.

```{r images-and-comments-ex-1}
question_text(NULL,
    answer(NULL, correct = TRUE),
    allow_retry = TRUE,
    try_again_button = "Edit Answer",
...
In the Terminal, run `ls images`. Th resulting list of files from the `images` directory should only contain one item: `car-plot.png`. CP/CR

```{r images-and-comments-ex-2}
question_text(NULL,
    answer(NULL, correct = TRUE),
    allow_retry = TRUE,
    try_again_button = "Edit Answer",
...
Add a code chunk and label it `map`. Load in the library **leaflet**. Run `cat "map-london.qmd"`. CP/CR.
