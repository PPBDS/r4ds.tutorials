...
It is a [flaw](https://github.com/quarto-dev/quarto-cli/issues/8540) in `quarto` that there is no easy way to provide a title to a new project created in the current directory.
...
Place your cursor in the at the `filter(z != 0) |>`. Hit `Command/Ctrl + Enter`. (Note that this should generate an error.) CP/CR.
...
Change the code so that the pipe, instead of just vomitting out its result for printing, assigns that result to a new object, named `D_diamonds`. You script should look like:
...
From the Console, run `list.files(patter = "clean")`. CP/CR.
...
From the Terminal, run `tail process_data.R`. CP/CR.
...
````
Davids-MBP:first-website dkane$ tail process_data.R
library(tidyverse)
...
In the Terminal, run `grep library make_plot.R`. CP/CR.

```{r using-a-png-file-ex-2}
question_text(NULL,
    answer(NULL, correct = TRUE),
    allow_retry = TRUE,
    try_again_button = "Edit Answer",
...
In the Terminal, run `tail -5 index.qmd`. CP/CR.

```{r using-a-png-file-ex-7}
question_text(NULL,
	answer(NULL, correct = TRUE),
	allow_retry = TRUE,
	try_again_button = "Edit Answer",
