...
This tutorial covers material from [Chapter 28 Quarto](https://r4ds.hadley.nz/quarto.html) and [Chapter 29 Quarto formats](https://r4ds.hadley.nz/quarto-formats.html) in [*R for Data Science (2e)*](https://r4ds.hadley.nz/quarto.html) by Hadley Wickham, Mine Çetinkaya-Rundel, and Garrett Grolemund, although much of the information from Chapter 28 has already appeared in the "RStudio and Github" tutorial in this package. We will go into much more detail with regard to the creation and organization of [Quarto websites](https://quarto.org/docs/websites/) using the [**quarto**](https://quarto-dev.github.io/quarto-r/) package and commands like `quarto_create_project()`, `quarto_render()`, `quarto_preview()` and `quarto_publish_site()`.
...
Second, we can use an R package like **quarto** directly. RStudio buttons are, generally, connected to R package commands. This is the approach we follow in this tutorial.

Third, we can work with Quarto directly at the Terminal. This is often referred to as *working from the command line*. R packages often call Terminal commands behind the scenes. Note that this requires installing the [Quarto CLI](https://quarto.org/docs/get-started/) yourself. We won't bother with this since Quarto comes installed in RStudio.
...
```
quarto_create_project(name = "my-cool-website", type = "website")
```

Respond with a "Y" when you are asked if you want to proceed. CP/CR.
...
There are two ways in which one might organize a website given that you are using Github. The first is to place all the necessary levels at the highest level, i.e., the level of the README.md file and other top level documents. The second approach is to create a new directory in which all the necessary files will be placed.

Because `quarto_create_project()` conflates the *name* of the project with the *location* of the project, there is no easy way to use the first approach.
...
````
> list.files()
[1] "first-website.Rproj" "my-cool-website"     "README.md"          
````

The basic set of files necessary for making a Quarto website has been added to the `my-cool-website` directory. 
...
In the Console, run `list.files("my-cool-website")`. CP/CR. 
...
```
> list.files("my-cool-website")
[1] "_quarto.yml" "about.qmd"   "index.qmd"   "styles.css"      
```

You would get the same set of files if, in the Terminal, you ran `ls my-cool-website`. 

### Exercise 7

From the Console, run `list.files(recursive = TRUE)`.
...
[1] "first-website.Rproj"         "my-cool-website/_quarto.yml" "my-cool-website/about.qmd"  
[4] "my-cool-website/index.qmd"   "my-cool-website/styles.css"  "README.md"     
```

For many commands, the answer you get depends on the *location* you are at when you issue the command. If you run `getwd()`, you will see that you are in the `first-website` directory. A simple `list.files()` will only list the files in that directory. It won't list the files in `my-cool-website`. However, if you `setwd("my-cool-website")` then you will now be in the `my-cool-website` directory and `list.files()` will give a different answer.
...
In the Console, run `list.files("my-cool-website", all.files = TRUE)`. CP/CR. 
...
```
> list.files("my-cool-website", all.files = TRUE)
[1] "_quarto.yml" "."           ".."          ".gitignore"  "about.qmd"   "index.qmd"   "styles.css" 
```

### 

You would get the same set of files if, in the Terminal, you ran `ls -a my-cool-website`. The `-a` ensures that **a**ll the files are listed, included the "hidden" ones which begin with a dot. Recall that `.` means the current directory while `..` means the directory one level above the current one.
...
From the Terminal, run `find . -name ".gitignore"`. CP/CR.
...
```
Davids-MBP:first-website dkane$ find . -name ".gitignore"
./my-cool-website/.gitignore
./.gitignore
Davids-MBP:first-website dkane$ 
```

In other words, there are two `.gitignore` files in your project, one at the highest level (created when the Github repo was connected) and one within `my-cool-website` (created with `quarto_create_project()`). 

It is common on larger projects to have more than one `.gitignore`. The `.gitignore` file in each directory applies to that directory and its subdirectories, unless overridden by another `.gitignore` file in a subdirectory below it.

### 

You could accomplish the same task from the R Console with this annoyingly like command:

```
list.files(path = ".", pattern = "\\.gitignore$", recursive = TRUE, full.names = TRUE, all.files = TRUE)
```


### Exercise 10

In the Console, run `setwd("my-cool-website")`. This changes the R session working directory to the `my-cool-website` directory. It makes no change in the working directory in the Terminal. These are completely separate processes from the computer's point of view.

Run `list.files()`. The "hidden" files will not appear because we did not include the `all.files = TRUE`. CP/CR.
...
In the Source pane, open the `_quarto.yml` file. Replace `title: "My-cool-website"` with `title: "A New Website"`. Save the file. 
...
With luck, future versions of `quarto_create_project()` will use the `dir` argument to specify the location of the new files, thereby allowing the `name` argument to be used for a sensible title for a newly-created project. In the meantime, we need to set our title by hand.
...
Open the `index.qmd` file in the Source pane. Replace `title: "My-cool-website"` with `title: "My First Website"`. Save the file.
...
Yes, it is a bother to have to replace `"my-cool-website"` twice. Very annoying! With luck, Quarto will soon allow us to specify a title when we first create these files. In the meantime, it is good to have an occasion to look closely at the files which help us to create a website.
...
Note that `.gitignore` contains only one line: `/.quarto/`. This `.gignore` was created when we ran `quarto_create_project()`.  The leading `/` escapes the `.`. In other words, we need the leading `/` if we want to ignore the files in a dotted directory.

In fact, the `.quarto` directory does not yet exist. But the `quarto_create_project()` command wanted to ensure that, once it does, Git would ignore it and its contents.


### Exercise 8

From the Terminal, run `pwd`. CP/CR.
...
You answer should be something like:

```
Davids-MBP:first-website dkane$ pwd
/Users/dkane/Desktop/projects/first-website
Davids-MBP:first-website dkane$ 
```

**Remember where you are.** In the R session, your current working directory is something like:

```
> getwd()
[1] "/Users/dkane/Desktop/projects/first-website/my-cool-website"
>
```

In the Console, you are in the "lower" directory because you moved there explicitly with `setwd()`. In the Terminal, you have not done anything like that, so you are still at the starting directory, which is one level up from the `my-cool-website` directory.


### Exercise 9

We don't want to put all the files on Github. So, add `*Rproj` to the `.gitignore` file which resides at the higher level, not the one in the `my-cool-website` directory. Save the file. 
...
From the Console, run `quarto_render()`. CP/CR.
...
In the "Background Jobs" tab, you should also see something like:
...
Add `/_site` to the `.gitignore` file at the *lower* level. Don't forget that the last line of a `.gitignore` should always be blank. Save the file. 
...
From the Console, run `quarto_preview()`. CP/CR.
...
The `quarto_preview()` command does two things. First, it renders the `qmd` files, just like `quarto_render()` does. Second, it sets up a viewer --- probably the "Viewer" tab in the Files pane, but possibly your default web browser --- so that you can see what the website looks like. Click around and check it out!
...
Look at your website in the browser. (You might need to enter the URL -- `http://localhost:6683/` in my case, but yours will be different, by hand.) Click on the "About" link. 

Now, go to the Console. Copy-and-paste everything which appeared after the previous invocation of `quarto_preview()`.
...
The advantage of using `quarto_preview()` is that the website is automatically updated whenever you edit and save the underlying QMD file. Try it by changing the `about.qmd` file, replacing "About this site" with your name. Save the file. 

At the Console, you should see a new message. Copy and paste it here.
...
If you want to stop `quarto_preview()`, issue the command `quarto_preview_stop()` in the Console.
...
* We recommend using your Github username and password. (It is much easier this way, since you don't have to juggle around a bunch of passwords and usernames when you want to publish.)
...
Ensure that all the files in the project have been committed and pushed. (Recall that you have, at least, edited `.gitignore` and `about.qmd`.)

At the Console, run `quarto_publish_site(server = "quarto-pub")`. You will then need to hit the "Enter" button twice, accepting the default options for account (your email address) and site name ("a-new-website"). CP/CR.
...
Form the Console, run `quarto_preview()`. Copy and paste the text from the home page once it comes up on your browser.
...
Stop the `quarto_preview()` command by hitting the red stop sign on the upper right of the Terminal tab. From the Terminal, run `quarto_publish_site()`. CP/CR.

You only need `quarto_publish_site()`, instead of `quatro publish quarto-pub` because the `_publish.yml` records that this project is published on Quarto Pub. You are simply refreshing the current project, rather than creating another.
...
In other words, RStudio figures that, if you are rendering an `index.qmd` which is part of a Quarto website project, you probably want to preview your new homepage. So, it calls `quarto_preview()` behind the scenes. The "Background Jobs" tab is where RStudio reports to you what is happening in the background.


### Exercise 4

In other words, you might not have to run `quarto_preview()` from the Console. Just rendering `index.qmd` directly is often enough. Copy and paste the new home page, leaving out the menu items.
...
There are two common modes for working with larger projects. First, as here, we work with an individual file, like `index.qmd`, and ensure that it is working as we want. Second, as before, we work with the project as a whole, using commands like `quarto_render()`, which render all the files in the project. The former approach is quickest and is how we spend most of our time. But, at the end of the day, we need to build the entire project.
...
In the Console, run `quarto_publish_site()`. You don't need to include `quarto-pub` since that information now appears in the `_publish.yml` file. You will need to hit "Enter" once.
...
Keep in mind that your browser now has two tabs which show your website. First, the `quartopub.com` version, which has not changed since you last hit `quarto_publish_site()`. The only way to affect the public website is to publish to it. Second, the `localhost` version, which is what you have been changing every time that you render `index.qmd`.
...
Push all the changes to Git, and issue the `quarto_publish_site()` command in the Console. You need to hit "Enter" at least once. CP/CR.
...
Run `quarto_preview()` from the Console. You will need to hit "Enter" once. Check out your new project! See how "Sources" now appears on the navigation toolbar. Click on "Sources" and see how the text you added to `sources.qmd` now appears on the web.
...
Run `quarto_preview()` to see the changes. 
...
Type `quarto_publish_site()` in the Console and hit "Enter." Explore the use of the "Sources" menu on your website.
...
In the Console, run `cat(readLines("make_plot.R"), sep = "\n")`. CP/CR.
...
From the Console, run `quarto_publish_site()`. You will need to hit "Enter" at least once. CP/CR.
...
This tutorial covered material from [Chapter 28 Quarto](https://r4ds.hadley.nz/quarto.html) and [Chapter 29 Quarto formats](https://r4ds.hadley.nz/quarto-formats.html) in [*R for Data Science (2e)*](https://r4ds.hadley.nz/quarto.html) by Hadley Wickham, Mine Çetinkaya-Rundel, and Garrett Grolemund, although much of the information from Chapter 28 had already appeared in the "RStudio and Github" tutorial in this package. We went into much more detail with regard to the creation and organization of [Quarto websites](https://quarto.org/docs/websites/) using the [**quarto**](https://quarto-dev.github.io/quarto-r/) package and commands like `quarto_create_project()`, `quarto_render()`, `quarto_preview()` and `quarto_publish_site()`.
